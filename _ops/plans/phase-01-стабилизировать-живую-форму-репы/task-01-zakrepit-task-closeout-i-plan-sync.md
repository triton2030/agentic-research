...
## Цель
`task-contract`, `project-strategy` и `instruction-layer` явно поддерживают полный цикл синхронизации задач и плана: обсуждение, работа, проверка критериев, closeout и root-instruction routing.

## Подшаги
- [x] Зафиксировать новый рабочий сигнал в `_ops/INTERVIEW.md`, `_ops/PROJECT-PLAN.md` и `_ops/learnings.md`.
...
- [x] Усилить `project-strategy`: сверка плана с историей чата, git evidence и фактически закрытыми task-файлами без превращения PROJECT-PLAN в индекс задач.
...
- [x] `task-contract` содержит явный completion closeout loop: после выполнения нетривиальной задачи агент снова вызывает skill и обновляет тот же task-файл. — **Evidence**: правка `projects/meta/task-contract--skill-codex/SKILL.md` и `references/task-file-lifecycle.md`.
  **Anchored in**: `_ops/INTERVIEW.md#Рабочий-Режим`
- [x] `project-strategy` использует фактическую историю как evidence для синхронизации плана: chat context, git diff/history и закрытые task-файлы, когда они доступны. — **Evidence**: правка `projects/meta/project-strategy--skill-codex/SKILL.md` и `references/plan-protocol.md` или `references/file-contracts.md`.
  **Anchored in**: `_ops/INTERVIEW.md#Рабочий-Режим`
- [x] `instruction-layer` требует, чтобы root instructions явно роутировали частый вызов `task-contract` для task-context discussion, edits, status changes и closeout. — **Evidence**: правка `projects/meta/instruction-layer--skill-codex/SKILL.md` / `references/workflow.md` и root docs.
  **Anchored in**: `_ops/INTERVIEW.md#Рабочий-Режим`
- [x] PROJECT-PLAN остаётся крупномасштабным планом, а не списком task-файлов. — **Evidence**: в правках есть запрет на task-index drift при чтении закрытых задач.
  **Anchored in**: `_ops/PROJECT-PLAN.md#Anti-goals`
- [x] Installed Codex skills не расходятся с repo source после правки. — **Evidence**: diff/validation показывает синхронность `/Users/triton/.codex/skills/task-contract`, `/Users/triton/.codex/skills/project-strategy` и `/Users/triton/.codex/skills/instruction-layer` с `projects/meta/*--skill-codex`.
  **Anchored in**: `_ops/PROJECT-PLAN.md#Stages`

### Must not
- [x] Project-strategy не редактирует содержимое task-файлов и не переносит acceptance criteria в PROJECT-PLAN. — **Why this would be bypassed**: удобно закрывать всё одним владельцем, но это ломает owner-boundary.
...
1. `python3 /Users/triton/.codex/skills/.system/skill-creator/scripts/quick_validate.py projects/meta/task-contract--skill-codex projects/meta/project-strategy--skill-codex projects/meta/instruction-layer--skill-codex`
...
2. `diff -ru --exclude README.md projects/meta/task-contract--skill-codex /Users/triton/.codex/skills/task-contract` и аналогично для `project-strategy`
   Expected: нет значимого diff после sync.
   Actual: `diff -ru` чистый для `task-contract`, `project-strategy`, `instruction-layer`; `ensure-ops.sh --check` — no drift.
