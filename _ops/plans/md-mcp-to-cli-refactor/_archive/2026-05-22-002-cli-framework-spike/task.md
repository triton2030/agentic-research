...
- [x] Выбрать 3 hardest tools для spike (variety of edge cases):
...
- [x] Implement spike в `experiments/md-embedding-server/spike/argparse_version.py`:
  - argparse + subparsers
...
- [x] Implement spike в `experiments/md-embedding-server/spike/typer_version.py`:
  - Typer commands (3 functions с decorators)
  - Same signatures, type validation through Python type hints
  - Auto-generated help
  - JSON output

- [x] Implement spike в `experiments/md-embedding-server/spike/catalog_driven.py`:
...
- [x] Verdict criteria (audit cycle-2 Smith G6 — catalog_driven это слой, не альтернатива):
...
- [x] Tie-breaker (audit cycle-2 Implementation G2):
...
- [x] Document decision в `experiments/md-embedding-server/docs/cli-framework-decision.md`:
...
## Готово
- [x] 3 spike реализации существуют (argparse, typer, catalog_driven)
- [x] Benchmark numbers зафиксированы
- [x] `docs/cli-framework-decision.md` существует с явным verdict
- [x] task-101 может использовать decision как input
