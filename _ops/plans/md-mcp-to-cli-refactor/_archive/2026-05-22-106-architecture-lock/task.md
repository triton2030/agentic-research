...
- [x] **MCP tool snapshot — generated, не hand-written**:
...
- [x] **catalog.py validation** против snapshot:
...
- [x] **Architecture boundary tests** `tests/test_architecture_boundaries.py`:
...
- [x] **Adversarial transaction tests** `tests/test_transactions_adversarial.py`:
...
- [x] **Lazy imports tests** `tests/test_lazy_imports.py`:
...
## Готово
- [x] `tests/golden/mcp-tool-snapshot.json` — 29 entries, generated from live MCP
- [x] `scripts/generate-mcp-tool-snapshot.py` существует
- [x] `tests/test_catalog_contract.py` — все assertions зелёные (29 entries + importability)
- [x] `tests/test_architecture_boundaries.py` — 4 sections (handlers/workflows/runner/library) все зелёные
- [x] `tests/test_transactions_adversarial.py` — adversarial core scenarios зелёные
- [x] `tests/test_lazy_imports.py` — help/list scenarios зелёные
- [x] `docs/architecture-lock.md` существует с invariants documentation
...
The lock tests enforce the new `md_cli` boundary and the new
`navigator.workflows` layer. They do not yet force legacy `navigator.cli` /
`navigator.graph` internals to stop printing JSON, because those files are the
old compatibility surface until Phase 2/202a finishes the public API split.

## Evidence

- `uv run pytest tests/test_catalog_contract.py tests/test_architecture_boundaries.py tests/test_transactions_adversarial.py tests/test_lazy_imports.py -v` → 17 passed.
- `uv run pytest tests/ -q` → 149 passed.
- `md tools --json | wc -c` → 48,524 bytes.
- `docs/architecture-lock.md` documents the gate and regeneration command.
