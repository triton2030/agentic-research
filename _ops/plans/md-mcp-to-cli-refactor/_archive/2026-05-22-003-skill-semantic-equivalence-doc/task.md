...
- [x] **Automation substep (audit cycle-2 Implementation G1)**: создать `experiments/md-embedding-server/scripts/extract-mcp-usages.py`:
...
- [x] Создать `experiments/md-embedding-server/docs/skills-semantic-equivalence.md`. Структура для каждого skill — одна секция с такими полями:
...
- [x] Заполнить **13 секций**, по одной на skill:
...
- [x] Для каждого skill — **verify** что CLI поддерживает все semantic patterns:
...
- [x] Особое внимание паттернам которые могут сломаться:
...
- [x] **Cross-platform проверка** в каждой секции:
...
- [x] **Section: «Что добавляется» (новое поведение)**:
...
- [x] Cross-reference в migration tasks (task-301/302/304/305):
...
- [x] Final review: запустить `1fresh-eyes` subagent с независимым взглядом для верификации:
  ...
  - **Pass criterion**: findings list (может быть empty). Все non-empty findings либо resolved правкой equivalence doc, либо записаны как accepted gaps в task-201/202/203/204 substeps или migration-task blockers (task-301/302/304/305) когда finding относится к live pre-migration skill text. Без этой routing — task-003 не закрывается.

## Готово
- [x] `docs/skills-semantic-equivalence.md` существует
- [x] Все 13 skills имеют секции с 6 полями каждая
- [x] Каждая секция cross-references task-201/202/203/204 для импленментации flags
- [x] Identified gaps (если есть) добавлены как substeps в task-201/202/203/204 или migration-task blockers
- [x] Independent subagent review completed; doc missing patterns resolved, live pre-migration skill gap routed to task-301/302/304/305
...
4. Independent subagent review проведён, findings reflected in task-201/202/203/204 or task-301/302/304/305 depending on owner.

## Evidence

- `python3 scripts/extract-mcp-usages.py --output docs/mcp-usages-extracted.csv` → 407 rows, 26 skill files represented.
- `grep -c '^## ' docs/skills-semantic-equivalence.md` → 13.
- `grep -c 'MCP usage' docs/skills-semantic-equivalence.md` → 13.
- `grep -c 'Semantic patterns preserved' docs/skills-semantic-equivalence.md` → 13.
- `_ops/findings/2026-05-22-equivalence-doc-review.md` records two subagent passes. Remaining F2 is not a doc gap: it is live pre-migration skill text routed to task-301/302/304/305 and task-204 transaction implementation.
