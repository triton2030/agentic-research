...
- [x] Реализовать `src/md_cli/handlers/selftest.py` (audit Implementation #6 — in-process vs subprocess):
...
- [x] **Acceptance criterion (precise)** (audit Smith #4 — vague boundaries):
  - Pass = exactly **28/29 OK + 1 skip (md_audit)**, или **29/29 если SMOKE_AUDIT=1 set**
  - НЕ ambiguous «29 или 28+1»

- [x] Portировать MCP smoke test fixtures:
...
- [x] Tests `tests/test_doctor.py`:
  - test: на текущей машине → ≥80% checks OK
  - test: `--json` shape

## Готово
- [x] `src/md_cli/handlers/selftest.py` реализован
- [x] `src/md_cli/handlers/doctor.py` реализован
- [x] `tests/fixtures/sample-corpus/` создан с indexed state
- [x] `tests/golden/selftest-expected.json` зафиксирован
- [x] `md selftest` returns 0 на fixture corpus (28/29 минимум, audit может быть skip)
- [x] `md doctor` runs, выводит понятный report
- [x] `tests/test_selftest.py` — зелёный
- [x] `tests/test_doctor.py` — зелёный
...
Phase 1 `selftest` is an architectural selftest because tool-specific handlers
are intentionally still stubs until Phase 2. It verifies catalog entries,
handler importability, target importability, dispatch and runner envelope. Phase
2 can upgrade individual tool probes to real behavior without changing the
`md selftest` interface.

## Evidence

- `md selftest` → `Pass: 28/29, Fail: 0, Skip: 1`.
- `md selftest --tool md_ping --json | jq '.results[0].status'` → `pass`.
- `md doctor --json | jq '.summary, [.checks[] | select(.status=="FAIL") | .name]'` → 0 fail.
- `uv run pytest tests/test_selftest.py tests/test_doctor.py tests/test_catalog_contract.py tests/test_md_cli_dispatch.py -v` → 15 passed.
