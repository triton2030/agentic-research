...
completed for Codex side. Stale `md_*({ ... })`, plain old `md_*` tool IDs,
`md-mcp`, `MD_NAVIGATOR_SCRIPT`, `md_navigator.py`, and `md_graph.py` refs are
gone from `/Users/triton/.codex/skills`. Extended updates landed in
`1ia-audit/references/md-evidence-probes.md`, `1instruction-layer`,
`1assumption-audit`, `1work-review`, `1skill-architect`, and
`1strategy/references/ground-check.md`; other listed Codex skills had no stale
refs in the current scan. Evidence:
`_ops/findings/2026-05-22-codex-skills-cli-migration.md`.
...
- [ ] **Transaction safety blocker from task-003 review**:
  - `~/.codex/skills/1smart-simple/SKILL.md` currently has
    `md_index({ corpus, dry_run: true })`, then
    `md_index({ corpus, confirm: true })`.
  - During migration, rewrite to CLI safety wording:
    `md index FOLDER --dry-run --json` → read returned `transaction_id` →
    `md index FOLDER --confirm --transaction-id <id> --json`.
  - Keep concrete semantic values: `md search ... --limit 3`, `md overlaps
    ... --threshold 0.85 --top 20`.
...
- [ ] `1smart-simple` no longer contains bare `md_index` confirm prose; it uses
  dry-run → `transaction_id` → confirm.
