...
- [ ] Скопировать `envelope-shape-snapshot.md` (from task-000) → `experiments/md-embedding-server/tests/golden/envelope_shape.json` как golden artifact.

- [ ] Реализовать `src/md_cli/envelope.py`:
  ...
  - `derive_next_step(result, tool_name, args, corpus_root)` — same logic as JS envelope.js:
    - `index_warmup_required` → 3 directives (md_index dry-run, md_index confirm, retry original)
    - `confirm_required` → 2 directives (dry-run, confirm)
...
- [ ] Реализовать `src/md_cli/cost_ledger.py` (audit Implementation #3 + Codex #9):
...
- [ ] Реализовать `src/md_cli/corpus_state.py`:
...
- [ ] Создать `tests/test_envelope_golden.py`:
  ...
  - test: known error pattern → `next_step` соответствует golden (index_warmup_required = 3 directives)
  - test: size_estimate.bytes большим object → `large_reply: True`
  - test: empty md_search result → broader scope directive

- [ ] Создать `tests/test_envelope_drift.py`:
...
## Готово
- [ ] `src/md_cli/envelope.py` существует, реализован полностью
- [ ] `src/md_cli/cost_ledger.py` существует с file-based persistence
- [ ] `src/md_cli/corpus_state.py` существует с 30s TTL cache
- [ ] `tests/golden/envelope_shape.json` зафиксирован
- [ ] `tests/test_envelope_golden.py` — все assertions проходят
- [ ] `tests/test_envelope_drift.py` — проходит, или явно skipped с reason (если drift expected на этом этапе)
...
3. `python3 -c "from md_cli.envelope import wrap; import json; print(json.dumps(wrap({'ok':True}, tool_name='md_ping', args={}), indent=2))"` — выводит JSON с `_envelope` блоком
