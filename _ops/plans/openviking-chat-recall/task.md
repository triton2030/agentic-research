...
| 6 | Переносимость | протокол по `RUNBOOK.md` применён к чужой папке `_ops/chat-recall` **без правки своего текста** — контрольный прогон уже после правки префикса |
| 7 | Защиты вместо внимательности | каждый класс ошибки сессии 2026-08-23 закрыт механизмом, который отказывает, а не напоминает |

### Веха 7 — что считается закрытым
