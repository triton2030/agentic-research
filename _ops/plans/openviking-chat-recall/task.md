...
| --- | --- | --- | --- |
| 7.1 | задание построено и не запущено | рукав `library` слепой приёмки пролежал файлом, пока три соседних считались | манифест волны сверяет три множества — построено · запущено · принято — и печатает разницу; пустая разница обязательна для отчёта волны |
| 7.2 | генератор заданий не помнит сделанного | `build_audit_tasks.py` выдал 40 заданий на 23 нужные темы; 17 сняты руками | каждый `build_*` принимает папку прогонов и по умолчанию пропускает уже принятые единицы; повторить всё — явный флаг |
