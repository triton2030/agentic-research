...
completed. 24 atomic handlers exist, stay thin through the shared generic
runner path, return `ToolResult`, and do not own envelope/JSON printing.
Verification: `uv run pytest tests/test_atomic_handlers.py
tests/test_mcp_cli_parity.py -q` passed; full suite 169 passed.
...
- [ ] Для **handlers требующих warm index** (md_overlaps, md_audit, md_repeated_concepts, md_search, md_query_by_type, md_refactor_candidates):
  ...
  - Runner intercepts exit_code=4 → envelope.next_step заполняет dry-run directive для `md_index` + retry original; runnable confirm появляется только после dry-run returned transaction_id/fingerprint
