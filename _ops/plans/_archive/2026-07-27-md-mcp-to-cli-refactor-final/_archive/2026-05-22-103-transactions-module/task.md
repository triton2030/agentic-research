...
- [x] Реализовать `src/md_cli/transactions.py`:
...
- [x] Cache file shape `~/.cache/md-tools/transactions/{txn_id}.json`:
  ```json
  {
    "id": "txn_abc123",
    "tool": "md_strip",
...
- [x] Intent-binding (S1 раunhinged): дополнительная защита поверх fingerprint.
...
- [x] Stateless fallback (без cache file):
...
- [x] Создать `src/md_cli/decorators.py`:
...
- [x] Tests `tests/test_transactions.py`:
...
## Готово
- [x] `src/md_cli/transactions.py` существует, все API функции реализованы
- [x] `src/md_cli/decorators.py` существует с `@requires_transaction`
- [x] Cache directory `~/.cache/md-tools/transactions/` создаётся при первом вызове
- [x] `tests/test_transactions.py` — все тесты зелёные, покрывают 6 сценариев
...
## Проверка
1. `cd experiments/md-embedding-server && uv run pytest tests/test_transactions.py -v` → 7/7 green
...
- `uv run pytest tests/test_transactions.py tests/test_envelope_golden.py -v` → 12 passed.
- `uv run pytest tests/ -v` → 126 passed.
- Tests cover transaction cache creation, success, drift, tool mismatch, expired token, stateless fingerprint fallback and args mismatch.
