...
- [x] Зафиксировать layout (см. секцию «Architectural anchor» выше):
  ```
...
- [x] Реализовать `src/md_cli/result.py`:
  ```python
...
- [x] Реализовать `src/md_cli/runner.py` — central envelope ownership:
  ```python
...
- [x] Handlers НИКОГДА не импортируют `envelope` напрямую. Только runner. Это enforced через grep test в `tests/test_architecture_invariants.py`.

- [x] Написать `pyproject.toml`:
...
- [x] Перенести `scripts/navigator/` → `src/navigator/` (git mv, сохранить историю).
...
- [x] Создать `src/md_cli/main.py` — dispatch с lazy import (audit Implementation #2):
  - Two-level dispatch:
  ...
  - Test: `uv run python -c "from md_cli.main import main; import sys; sys.argv=['md','--help']; main()"; print('networkx in modules:', 'networkx' in sys.modules)` → False
  - Это NON-trivial argparse pattern; possibly use Click groups или Typer (per task-002 verdict)

- [x] Создать `src/md_cli/handlers/__init__.py` пустой. Handlers сами добавляются в Phase 2.

- [x] Добавить базовые универсальные flags на root parser:
  - `--version`
  - `--help` (auto)
  - `--json` (передаётся в subcommands)
  - `--corpus PATH` (если applicable, по convention — positional, не flag, но some tools его share)

- [x] Установить локально editable:
...
- [x] Добавить в `experiments/md-embedding-server/tests/test_md_cli_dispatch.py`:
...
## Готово
- [x] `experiments/md-embedding-server/pyproject.toml` существует, валиден (`uv tree` не падает)
- [x] `src/navigator/` существует, старые тесты в `tests/` зелёные после move
- [x] `src/md_cli/main.py` существует, argparse dispatch работает
- [x] `uv tool install --editable` ставит `md` в PATH
- [x] `md --version` → `0.7.0`
- [x] `md --help` показывает 29 subcommand names
- [x] `test_md_cli_dispatch.py` — все тесты зелёные
- [x] Lazy import проверен: `md status` не impl-ит NetworkX до handler load
...
5. `uv run python -c "import sys; from md_cli.main import main; sys.argv=['md','status','--help']; main()"` — не подгружает `networkx` (`'networkx' not in sys.modules`)

## Evidence

- `uv run pytest tests/ -v` → 110 passed.
- `uv run --frozen pytest tests/test_md_cli_dispatch.py -v` → 4 passed.
- `uv tool install --editable .` → installed executable `md`.
- `md --version` → `md-tools 0.7.0`.
- `md --help` lists 29 subcommands.
- `scripts/md_navigator.py --help` and `scripts/md_graph.py --help` still work after the `src/` move.
