...
completed. `md_init`, `md_strip`, `md_index`, and LLM/auto
`md_profile_sections` are guarded by dry-run → `transaction_id`/`fingerprint`
→ confirm. CLI signatures now expose `--fingerprint` as the stateless fallback.
Verification: `tests/test_mutating_handlers.py`,
`tests/test_mutating_mcp_parity.py`, `tests/test_transactions_adversarial.py`,
and full suite passed (169 tests).
...
- [ ] Tests `tests/test_mutating_handlers.py`:
  - test md_init без флагов → exit 1 with `confirm_required` + envelope.next_step has dry-run directive only
...
- [ ] `tests/test_transactions_adversarial.py` — args mismatch, double confirm, concurrent confirm, corrupt txn file, PermissionError, affected-set drift
...
## Проверка
1. `md init --path /tmp/test --json` (без флагов) → exit 1, `_envelope.next_step[].tool == "md_init"`, dry-run directive only
2. `md init --path /tmp/test --dry-run --json | jq '.transaction_id'` → exists
3. `md index --corpus /tmp/empty --json` → exit 4, error `index_warmup_required`, envelope.next_step has dry-run + retry original, no runnable confirm
4. `cd experiments/md-embedding-server && uv run pytest tests/test_mutating_handlers.py tests/test_mutating_mcp_parity.py tests/test_transactions_adversarial.py -v` → all green
