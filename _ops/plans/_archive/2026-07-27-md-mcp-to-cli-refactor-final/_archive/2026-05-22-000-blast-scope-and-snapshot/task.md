...
- [x] Зафиксировать MCP registration paths. Найти где `md-mcp` зарегистрирован для Claude/Codex.
...
- [x] Inventory 29 tool signatures как single source of truth перед миграцией.
...
- [x] Snapshot envelope shape (`envelope.js`).
...
- [x] Inventory всех cross-project references на md_* tool names.
...
- [x] **Snapshot real MCP JSON responses** (audit fix Smith #5 + Implementation #7):
...
- [x] Создать git tag `pre-mcp-refactor-2026-05-22` на текущем main как rollback point.
  - `git tag pre-mcp-refactor-2026-05-22 main`
  - Не push в remote — локальная safety net

- [x] Подтвердить что smoke test проходит на baseline.
...
## Готово
- [x] `_ops/findings/2026-05-22-mcp-registration-locations.md` существует и перечисляет все места регистрации.
- [x] `docs/tool-signatures-snapshot.json` существует и содержит 29 tool definitions.
- [x] `docs/envelope-shape-snapshot.md` существует и golden reference envelope shape.
- [x] `docs/migration-blast-inventory.md` существует и перечисляет каждый файл с количеством refs (13 skills + repo docs).
- [x] `tests/golden/mcp-responses/<tool>.json` — 29 файлов с volatile fields stripped.
- [x] git tag `pre-mcp-refactor-2026-05-22` создан локально.
- [x] `npm run smoke` зелёный 38/38; `md_audit` included in generated snapshot.
