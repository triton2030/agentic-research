...
Практический фокус проекта — полигон под `GPT-5.6` и `Claude Opus 4.7`: здесь
...
- Использовать `_ops/plans/**` только по явному запросу для активной сложной
  работы.
- Сверять рабочий канон с модельной парой `GPT-5.6` / `Claude Opus 4.7`.
...
- Instruction file или skill начинает владеть тем, что должен держать
  `_ops/GOAL.md`.
- Нужно расширить модельную рамку за пределы `GPT-5.6` / `Claude Opus 4.7`.
