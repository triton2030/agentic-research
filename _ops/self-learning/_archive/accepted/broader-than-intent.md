...
- 2026-05-20 [GPT-5.5]: после обсуждения Claude/Gemini MCP user сказал
  «исправь всё». Я начал с runner-code правок (`final-output.md`,
  `agent_behavior`) до явного отделения reusable product workflow от
  инженерного слоя. User вызвал `$1step-back`; правильная рамка уже была
  уже: сначала единый External Peer Review сценарий в скилах, потом только
  минимальный код, который снимает доказанную боль.
- 2026-05-20 [GPT-5.5]: когда user подтвердил narrowed vertical path, работа
  пошла лучше: runner smoke → skill contract → global sync → tail checks. Урок:
  после широкого «исправь всё» сначала назвать вертикальный reusable workflow,
  а затем выполнять только код/тексты, которые доказывают именно его.
