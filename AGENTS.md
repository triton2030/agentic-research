...
- **Снятое уходит в `_archive/` внутри своей папки** вместе с одной строкой о
  том, чем оно перестало быть правдой. Живая база снятого не содержит, и агент
  не ходит туда за маршрутом.
- **Эталон чужого артефакта сохраняется дословно**, включая его имена файлов:
  форма исходника — часть предмета изучения. Наш разбор лежит рядом отдельным
  файлом и эталон не переписывает.
