...
| `_ops/chat-recall-topics/` | то же знание по сорока предметам, якорь на реплику | `_ops/chat-recall-topics/AGENTS.md` |
...
- **За позицией владельца иди сперва в `_ops/chat-recall-topics/`.** Слой тем —
  ...
  ведёт в сырой корпус, здесь — сначала слой, потом реплика по якорю пункта.
  Доказательством остаётся реплика, не пункт: слой обзорный, без даты и без
  отменённого. Контракт слоя и его обновление — `_ops/chat-recall-topics/AGENTS.md`.
