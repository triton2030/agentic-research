...
(cross-project, veto-class), пиши project-independent; репо — источник правды для
скилов, не наоборот. `experiments/**` — независимые под-проекты со своей правдой.

## Цель

Делать систему `skills`, hooks, prompts, инструкций, знаний и рабочих контрактов
понятнее для будущей ИИ-сессии под `GPT-5.6` и `Claude Opus 4.7`. Канон пишется
только под эту пару; `GPT-5.5` и более старые OpenAI-модели остаются только
историческим evidence, не рабочим target или fallback.
