...
- **Единого лимита нет:** тип инструкции и метрика меняют ответ на порядки.
- **Разнотипные active constraints, strict all-pass:** CSE опускается ниже 50%
  при `k=7` у GPT‑5.5, `k=6` у Claude Opus 4.7 и `k=3` у GPT‑5.4 Pro и
  DeepSeek V4 Pro.
- **Однотипные lexical requirements, marginal accuracy:** GPT‑5.5 сохраняет
  около 99% выполнения отдельных требований вплоть до `N=5 000`; это не
  означает идеального выполнения всего набора.
- **Простые правила, strict perfect response:** Sonnet 5 выполняет все 20 в
  75.0% ответов, 40 — в 23.8%, 80 — в 0%.
- **Длинная агентная работа:** лучший результат HANDBOOK.md — 36.2% strict pass
  при среднем 12.7 критериев и примерно 30 tool calls; безопасный cap не найден.
- **Практическое решение:** `0–5` активных разнотипных обязательств — стартовая
  зона для проверки, `6–7` — зона перелома, `8+` требуют декомпозиции или
  validator. Число 20 не является доказанным пределом памяти или надёжности.
