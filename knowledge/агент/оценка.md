...
проверять траекторию, не только финал. Runtime guarantees — `knowledge/агент/слой исполнения.md`.
Memory effectiveness — `knowledge/агент/память.md`.
...
- `knowledge/скил/`
  Канон, приёмка, различия платформ и внешние срезы — против чего проверять
  собственную оценку.
