...
Модельный baseline держит `knowledge/wisdom-claude-opus-4.7.md`. В этом
практическом guide оставлять только Claude Code packaging и installed-skill
правила.
...
- Настойчивость trigger не должна расширять область скилла за пределы явно
  названного класса задач.
...
- Quick reference, команды, QA loop и anti-pitfall формулировки (`CRITICAL`, `ALWAYS`, `NEVER`) — нормальный официальный паттерн, если они закрывают verified failure mode. Не тащить старые QA/self-check ритуалы без baseline.
...
Документацию для человека держать в `knowledge/`, не в установленном скилле.
...
- Не компенсировать неверный runtime-режим текстом скилла.
