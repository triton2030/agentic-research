...
3. Выбери механизм по классу правила, а не по слабости:
   instruction < skill < validator < hook < permission/sandbox/CI/server policy.
   Для **предпочтения** иди снизу — самое слабое, что держит. Для **инварианта**
   стартуй сверху: две нижние ступени замерены в 0–4% соблюдения без
   предъявляемого артефакта, а изменение набора доступных инструментов подняло
   соблюдение с 0% до 74.7% при неизменном тексте. Совпадает со строкой ниже:
   знание — в skill, инвариант — в исполняемый слой.
