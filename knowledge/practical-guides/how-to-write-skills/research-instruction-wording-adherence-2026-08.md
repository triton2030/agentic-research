...
Срез источников на 26 августа 2026. Использовать при создании и аудите skills,
...
- Constraint Saturation Evaluation (CSE) систематически варьирует `k=1–12`: 36
  детерминированно проверяемых типов ограничений, 15 моделей и 369 753 проверки.
  У GPT-5.5 strict all-pass падает ниже 50% при `k=7`, у Claude 4.7 Opus — при
  `k=6`; у 12 из 15 моделей — при трёх или меньшем числе ограничений. В отличие
  от Instruction Stacking Collapse, CSE находит почти независимые отказы, а не
  сильное попарное подавление: мягкое падение надёжности каждого ограничения
  мультипликативно обрушает вероятность выполнить все. Это свежий
  нерецензированный препринт; названные модели не являются текущим target set.
...
- В VeyraBench Sonnet 5 прошёл все 10 простых правил в 93.8% ответов, 20 — в
  75.0%, 40 — в 23.8%, 80 — в 0%. Большинство наращиваемых правил были
  include/forbid-word constraints. Поэтому эти числа не противоречат CSE и не
  задают общий предел: разная атомарность и стоимость правил дают разные кривые.

**Authoring consequence:** считать не символы и не «единицы знаний», а активные
обязательства, длительность их удержания, зависимости и конфликты. Новую строку
принимать только если она закрывает отдельный failure mode; при конфликте
удалять или сливать старое правило, а не добавлять третье.

### Информация И Ограничение — Не Одна Единица Нагрузки

Прямой вывод исследований уже сильнее метафоры «модель держит 10–20 единиц в
голове»:

- CSE считает **активные ограничения**, которые надо одновременно понять,
  удерживать во время генерации и выполнить. Он не измеряет пассивные факты,
  которые могут ни разу не понадобиться.
- Внутри самих ограничений стоимость различается. Структурные ограничения в CSE
  теряли baseline capability примерно вдвое быстрее лексических. Требования с
  непрерывным контролем — считать слова, избегать буквы, поддерживать диапазон
  длины — деградировали быстрее бинарных решений вроде обязательного слова или
  JSON-структуры.
- VeyraBench отдельно измеряет recall факта из контекста. Там проблема — найти
  нужный фрагмент; до 64k–128k токенов recall у моделей часто остаётся близким к
  потолку, а около эффективного context ceiling резко зависит от модели и
  формата. Это retrieval failure, не тот же объект, что joint constraint
  failure.
- Harness-IF переносит одно и то же operational rule между system prompt, tool
  description, skill description, project file и user instruction. В
  конфликтном пилоте pooled precedence не следовал глубине prompt: system,
  project и user surfaces были выше tool и skill descriptions. Однако main
  per-surface rates нельзя читать как чистый рейтинг поверхностей: admissibility
  различалась, а user arm состоял только из against-prior rules.
- В Harness-IF каждую модель правила против её наблюдаемого дефолта снижали
  accuracy на 3.6–7.4 п.п. (среднее 5.81). Значит, одинаковая по длине строка,
  совпадающая с default модели, дешевле строки, которая должна default подавить.

Рабочая модель нагрузки:

```text
факт: доступ → извлечение → применение один раз
правило: доступ → распознавание применимости → удержание → применение → проверка
процедура: несколько связанных правил + состояние порядка и завершения
```

Один факт становится поведенческой нагрузкой только в той мере, в какой из него
надо регулярно выводить действие. «Тесты лежат в `tests/`» — справочная запись;
«перед каждым изменением запускай тесты из `tests/`» — устойчивое обязательство.
Один абзац процедуры может содержать несколько атомарных обязательств, а одна
короткая цель — сжимать много решений без требования воспроизводить весь путь.
...
5. Считают одновременно активные обязательства, их maintenance cost,
   зависимости, расхождение с default и конфликты, а не строки, символы или все
   содержащиеся в контексте факты.
...
- Нет универсального character cap или безопасного числа правил для любого
  agent/runtime.
- Нет evidence для однородного лимита «10–20 единиц знаний»: instruction-count
  benchmarks считают активные verifier-checked constraints, а long-context
  benchmarks — retrieval фактов; их числа не складываются.
...
- [Large Language Models Can Follow Instructions, But Not Many at Once / CSE](https://arxiv.org/abs/2608.12426)
  — `k=1–12`, strict/marginal satisfaction, типы ограничений и
  comprehension-maintenance gap; препринт, 2026.
- [Harness-IF](https://arxiv.org/abs/2608.11727) — operational rules на пяти
  configurable surfaces, against-prior control и conflict precedence;
  препринт, 2026.
