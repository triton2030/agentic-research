...
может пересказать правило и всё равно нарушает его, разрыв дефолта реален
([KBV](https://arxiv.org/abs/2604.28031)). Но правило само его **не закрывает**:
замер с одинаковым текстом даёт 0–4% соблюдения без предъявляемого артефакта и
97% с ним. Правило остаётся **вместе с артефактом**, который читает независимая
сторона; голое правило — не решение, а его имитация. Добавлять то, что
