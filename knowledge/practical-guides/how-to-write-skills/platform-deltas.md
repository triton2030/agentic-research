...
MCP даёт доступ к инструментам и данным. Skill добавляет недостающий контракт:
какой outcome получить, по каким constraints выбрать capability, какое evidence
нужно вернуть и где остановиться. Конкретный tool или порядок фиксируй только
когда от него зависит совместимость, authority, safety или корректность.
