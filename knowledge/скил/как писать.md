...
- остаётся в portable ceiling 1024 символа; platform-specific discovery limits
  и shortening держит `knowledge/скил/где расходятся платформы.md`.
...
Общий принцип — `knowledge/модель/свойства рабочего набора.md`: подразумеваемое держи как критерии
...
Claude-specific context assembly держит `knowledge/рантайм/claude code.md`.
...
Orchestrating skill обязан назвать контракт делегирования целиком; сам контракт
живёт у координации нескольких агентов, и переписывать его в скил не нужно.
