...
- Статичная структура instruction-файлов сама по себе не доказана как главный
  рычаг adherence: факторный эксперимент 2026
  ([2605.10039](https://arxiv.org/abs/2605.10039)) не нашёл надёжного эффекта от
  size, position, architecture и adjacent conflicts. Это не опровергает
  конкретный restatement в точке действия: файл без повторного чтения эффекта
  не даёт, а возвращённое у развилки правило является отдельным вмешательством.
  Сначала чинить minimal requirements, owner clarity и validation.
