---
description: "Правила, меняющие работу именно с GPT-5.6: brief, усилие, scope, делегирование."
aliases: []
---

# GPT-5.6

Снимок на 26 августа 2026.

Здесь живут только правила, которые важны именно для `GPT-5.6`. Общие свойства
LLM держит `knowledge/модель/свойства рабочего набора.md`; платформенные правила Codex держит
`knowledge/рантайм/codex.md`.
...
- Один пользовательский prompt не означает один внутренний шаг: GPT-5.6 может
  запускать programmatic tools, multi-agent и persisted reasoning. Поэтому
  простоту запроса и wall-clock duration нельзя принимать за меру сложности;
  outcome оценивается по завершённой проверке результата. [Официальная
  guidance](https://developers.openai.com/api/docs/guides/latest-model) описывает
  эти возможности, но не задаёт универсальную кривую надёжности по длине
  trajectory.
...
- `knowledge/инструкция/как писать/системный промпт.md` — при написании системных промптов.
- `knowledge/инструкция/как писать/сборка контекста.md` — при сборке контекста.
- `practical-guides/how-to-write-skills/` — при authoring skills.
- `knowledge/рантайм/codex.md` — когда GPT-5.6 работает внутри Codex-подобного coding agent.
