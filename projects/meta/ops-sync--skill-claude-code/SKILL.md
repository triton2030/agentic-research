...
Create or synchronize phase folders from `PROJECT-ROADMAP.md`. Keep `_ops/plans/phase-NN-<slug>/` and `done/` present for each Stage. Never write task-file content.
...
1. Confirm `PROJECT-ROADMAP.md` exists and has Stages.
...
4. Route plan/status meaning changes to `project-roadmap`.
...
Next route: <none/project-roadmap/task-contract>
```

## Role Boundaries

- file writes only `_ops/plans/` folder shape.
- Does not edit `PROJECT-ROADMAP.md`, `INTERVIEW.md`, `learnings.md`, or task files.
