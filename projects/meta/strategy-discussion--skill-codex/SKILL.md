...
description: >
  ...
  route to `project-roadmap`, `task-contract`, `user-truth`, `before-work`, or
...
and routes it to the owner: `project-roadmap` for direction, `task-contract`
for task scope, or `before-work` when execution can start.

Do not maintain a competing planning map here. The canonical level contract
lives with `project-roadmap`; this skill only decides which owner should receive
the selected route.
...
6. Route next:
   ...
   - Scope is stable enough for task criteria -> `task-contract`.
   - Implementation path is stable and work may start -> `before-work`.
...
**Routes to:** <project-roadmap | user-truth | task-contract | before-work | execute>
