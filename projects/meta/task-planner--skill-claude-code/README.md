...
Клодовская версия скилла `task-planner`. Рядом лежит кодексовская версия: `projects/meta/task-planner--skill-codex/`.
...
Если `_ops/PROJECT-PLAN.md` и `_ops/INTERVIEW.md` существуют, считает их upstream картой от `main-strategy`. Критерии приёмки — Must / Must-not / Verification protocol, каждый `Must` несёт `Anchored in:` со ссылкой на секцию плана / интервью, либо явную метку `local-only — <reason>` (разрешена только для критериев о самом коде). Это goal-alignment механизм, не общее "усиление критериев".

## Ключевые Правила

- **Один файл на Step**. Путь задан: `_ops/plans/phase-NN-<slug>/task-MM-<slug>.md`. Сам скил папки фаз не создаёт — ими владеет `main-strategy`.
- **Plan-Anchor Gate (блокирующий)**: если задача не якорится ни в одном элементе PROJECT-PLAN — откат в `main-strategy`. `local-only` как обход запрещён.
- **Эфемерный слой**: `_ops/plans/` может быть переставлен или удалён `main-strategy` целиком. Ничто снаружи не должно цитировать пути внутри `_ops/plans/`. `Anchored in:` ссылается только на `PROJECT-PLAN.md` или `INTERVIEW.md`.
...
Во время исполнения скил пишет **только** task-файл. `_ops/PROJECT-PLAN.md`, `_ops/INTERVIEW.md`, `_ops/learnings.md` — зона `main-strategy`, сюда не пишет.

## Куда Деплоить

В `~/.claude/skills/task-planner/` для персонального Claude Code skill, либо в `.claude/skills/task-planner/` внутри проекта.
