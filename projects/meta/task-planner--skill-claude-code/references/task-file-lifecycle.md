...
Default-режим `task-planner`. Пять чекпоинтов. Артефакт — файл `_ops/plans/phase-NN-<slug>/task-MM-<slug>.md` + компактный receipt в чат.
...
- Если папки `_ops/plans/phase-NN-<slug>/` нет, хотя Stage в плане есть — **стоп**. Это рассинхронизация планового слоя, владелец `main-strategy`. Emit: *«Папка фазы отсутствует. Вызываю `main-strategy` для синка, потом возвращаюсь»*.
...
Не контрабанди нерешённую архитектуру в `Must`. Если контракт зависит от решения, где живёт правило — остановись и откатывай в `system-architect`.
