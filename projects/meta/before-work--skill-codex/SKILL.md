...
name: before-work
description: >
  Use before non-trivial work starts: implement, fix, edit, write, continue, or create artifacts. Refresh Stage, goal, Must-not, next step. Route missing task contract to `task-contract`, missing plan anchor to `project-strategy`. Skip typos, facts, review, preference talk, and imminent write checks.
---
...
Это **лёгкий preflight**, не owner-skill. Не пишет файлы. Не ведёт план. Не задаёт interview-вопросов. Один проход за пять-десять секунд работы Codex — освежает рамку и возвращает к задаче.
...
Это защита от паттерна «memory says X exists → Codex builds X by analogy without checking». Особенно опасно для agent / skill / hook or validator where the runtime supports it слоёв — там разная runtime-механика, аналогия по описанию даёт неправильную форму.
...
- **Preference / direction / exploration talk без императива.** «Хочу попробовать», «хочу обсудить», «давай подумаем», «как лучше», «куда дальше», «что думаешь о подходе» — это `project-strategy` / `preference-sync`, не preflight. Триггер требует **императивной формы старт-работы** (сделай / напиши / реализуй) или приближающегося `file edit`/`file write`, не упоминания глагола в инфинитиве.
...
- Я выбрал layer (skill / agent / hook or validator where the runtime supports it) для нового артефакта по аналогии «по мотивам существующего», не сверившись где живёт паттерн в реальности → конфликт runtime-форм.
