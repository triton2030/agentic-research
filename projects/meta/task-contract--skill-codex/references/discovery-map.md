...
Extended patterns for Step 2 (Discover) of `task-contract`'s task-file lifecycle. Use this file only after the default probe list in `SKILL.md` is already understood.
...
If `CWD/_ops/PROJECT-PLAN.md` and `CWD/_ops/INTERVIEW.md` exist, prefer them early in discovery for major execution tasks. Treat them as upstream truth from `project-strategy`, then translate only the parts that materially change the contract.

If `CWD/_ops/PROJECT-PLAN.md` is missing, stop task-file creation and hand off to `project-strategy`. Do not keep going with a `local-only` contract as a substitute for an approved plan.
...
Before widening into local sources, prove that the ask serves the current `Goal`, active `Stage`, and either a current `Step` or a clear in-trajectory implication from the approved plan. If you cannot show that, stop and route back to `project-strategy` instead of inventing criteria from local context.
...
If `_ops/PROJECT-PLAN.md` exists, check whether the feature supports or contradicts the current `Goal`, `Approach & Why`, active `Stage`, and `Anti-goals`. If it is not clearly on the current trajectory, stop and route to `project-strategy` before emitting criteria.
...
If `_ops/PROJECT-PLAN.md` exists, use its optional `Anti-goals` section to guard against "clean refactor, wrong direction" outcomes. If the refactor is not justified by the current plan trajectory, stop and route to `project-strategy`.
