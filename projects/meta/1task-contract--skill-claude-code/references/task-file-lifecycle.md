...
`PROJECT-ROADMAP.md` has no Steps or status checkboxes. Do not ask strategy
to provide them.
...
Do not create phase folders. `ops-sync` owns folder shape.
...
- Write-back: if execution discovers required work outside the current
  `Подшаги`, update this task-file via `1task-contract` before continuing.
...
- **Orphan:** still relevant, not closed in this task. Leave open and emit a
  handoff back to `1task-contract` with the decision: new task-file or migration
