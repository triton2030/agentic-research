...
Порядок предпочтения при проектировании защиты: **runtime (hook or validator where the runtime supports its or validators where the runtime supports them/tool tool permission / approval policys / approval policy) → local skill → instruction text → 1task-contract handoff → human checkpoint**. Prompt-level правила — последнее средство, не первое.
...
Примеры: `project-strategy` (thinking workflow для durable bet), `1task-contract` (task-level контракт), TDD, security review.
...
Примеры: `1instruction-layer` (audit + design), frontend-design, research audit.
...
5. **1task-contract** ловит task-level bypass.
