...
Для Codex-specific механизмов (hook or validator where the runtime supports its or validators where the runtime supports them, tool tool permission / approval policys / approval policy, Codex Codex subagents, MCP) — см. соседний файл `claude-code-guardrails.md`.
...
- **Preference order fix-layers**: runtime guardrail (hook or validator where the runtime supports it/tool permission / approval policy/validator) → local skill → instruction text → `1task-contract` handoff → human checkpoint. Prompt-level прескрипция — последнее средство, не первое.
- **Plan-specific vs generic failure modes.** Generic failure modes (см. «Частые Симптомы» ниже) важны, но недостаточны. Каждый релевантный Stage плана генерирует свой класс ошибок. Источник plan-specific failures — `_ops/learnings.md` (реальные дельты) + внутренний inversion/premortem архитектора по активному Stage. Архитектор обязан превратить их в структурные защиты.
...
- `Runtime layer важнее текстовых просьб`
  Для рискованных действий опирайся на schema, validation, approvals, hook or validator where the runtime supports its or validators where the runtime supports them, sandbox и post-action checks. Prompt-only защита хрупка. *(Bezos one-way / two-way doors: необратимые действия требуют structural, а не текстового guard'а.)*
...
- `Модель выбирает самый короткий путь`
  ...
  Обычно чинится через: жёсткие критерии принятия и `1task-contract`, validation loop, наблюдаемые evidence artifacts.
...
|---|---|---|---|
| **Структурная** (always-on) | `1instruction-layer` | Автоматически в каждой сессии | PreToolUse hook or validator where the runtime supports it блокирует file edit в `node_modules`; tool permission / approval policy `Bash(rm -rf *): deny`; validator на схему commit message |
| **Task-specific** (on-demand) | `1task-contract` | Когда явно вызван под задачу | Must-not «не менять legacy X без approval»; Must «добавить regression test»; verification-шаг «проверить результат в браузере» |
...
**Правило эскалации:** если `1task-contract` раз за разом пишет **одну и ту же** Must-not — это сигнал, что правило должно жить в hook or validator where the runtime supports it/validator/tool permission / approval policy, не в receipt. Эскалация обратно в `1instruction-layer` — собственно, это один из входов этого скила.
...
3. Это сбой определения done?
   Тогда нужен `1task-contract` и более жёсткие acceptance criteria.
4. Это опасный или необратимый сбой?
   Тогда переносить контроль в runtime layer: validation, hook or validator where the runtime supports it, approval, sandbox.
