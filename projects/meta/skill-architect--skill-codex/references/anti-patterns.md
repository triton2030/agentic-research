...
- **Prescription без механизма обновления триады.** Если в output нет ни одного hook or validator where the runtime supports it / skill / checkpoint, который триггерит запись в `_ops/` при preference revealed / plan delta / expected-vs-actual дельте — архитектор оставил upstream без защиты. Это failure, не feature.
...
- **Ожидать, что `project-strategy` сам вызовется при каждом сигнале.** `project-strategy` — процедура, а не fairy. Её вызов должен быть триггернут механизмом (user prompt handling hook or validator where the runtime supports it на preference keywords, Stop hook or validator where the runtime supports it на Stage completion), а не надеждой.
...
- **Prescription без reuse-first gate.** «Добавить hook or validator where the runtime supports it X» без строки *«существующее покрытие <handle> недостаточно, потому что Y»* = изобретение вслепую.
...
- **Одному правилу в трёх местах без owner'а.** `_ops/` + `AGENTS.md` + skill + hook or validator where the runtime supports it на одну проблему — drift-источник. Один owner или compose layer.
...
- **Предположению о capability по текстовому упоминанию.** Проверяй реальную установку (`config/instruction files`, `installed_plugins.json`), не «в README написано — значит есть». Subagent probe или прямая проверка.
- **Внешнему поиску до локального capability audit.** Обычно решение уже лежит в репо — не увидел, потому что не посмотрел.
- **Построению нового skill'а, когда Codex skill folder уже содержит подходящий.** Reuse-first gate блокирует это.
...
- **Закладке абстракций «на всякий случай».** `PROJECT-PLAN.md` покрывает всю траекторию — используй это, чтобы **удалять** сложность, а не готовиться к воображаемому будущему. Default — YAGNI; знание плана даёт право на упрощение, а не мандат на future-proof.
