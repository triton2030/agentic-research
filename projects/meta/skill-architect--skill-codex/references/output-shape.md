...
- PROJECT-ROADMAP §Stage (активный): <1 строка — заполнен/generic/отсутствует>
...
### Skills
- `<имя>` (<Codex skill folder|user|project>) — triggers: <...> — покрывает класс <N>

### MCP servers
- `<имя>` (<scope>) — реально доступные `mcp__*` prefixes: ...

### Codex Codex Codex subagents
...
- Если prescription рекомендует конкретный Codex-механизм — ссылайся на [claude-code-guardrails.md](claude-code-guardrails.md), не переписывай теорию.
