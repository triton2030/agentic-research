...
- **Hooks** — `Codex configuration/instructions` (project scope) + `~/Codex configuration/instructions` (user scope). Event × matcher × действие. Включая: PreToolUse, PostToolUse, user prompt handling, Stop, fresh-session instruction load.
- **Permissions** — allow / deny / ask правила, scope каждого.
- **Skills** — Codex skill folder (установленные плагины), user-global (`~/.claude/skills/` и `~/.claude/Codex skill folders/*/skills/`), project-local (`projects/*/SKILL.md` и `.claude/skills/`). Триггеры и границы, не только имена.
- **MCP servers** — зарегистрированные, в каком scope, реально доступные `mcp__*` tool prefixes.
- **Codex Codex Codex subagents** — определённые, с `allowed-tools`.
...
- **Точные handles, не классы.** Не «hook or validator where the runtime supports its or validators where the runtime supports them есть», а «PreToolUse на file write, matcher `_ops/**`, blocks with exit 2».
- **Реальная установка, не текстовое упоминание.** README может врать. `config/instruction files` и `installed_plugins.json` — нет. Сомневаюсь — Codex subagent probe с минимальным prompt'ом, проверяю поведение.
...
- **Constraint** — что я из-за этого **не** буду проектировать (*«не фиксирую имя текущей модели в hook or validator where the runtime supports it matcher'е — через квартал смена, hook or validator where the runtime supports it сломается»*).
...
- **Где система сейчас это позволяет** — конкретное слабое место в instruction layer / hook or validator where the runtime supports its or validators where the runtime supports them / правах.
...
- Сигнал «preference revealed» приходит — что заставляет записать в INTERVIEW? user prompt handling hook or validator where the runtime supports it? Правило в AGENTS.md? Или ничего, и запись происходит только если пользователь сам попросит?
- Сигнал «plan delta» приходит — что заставляет обновить PROJECT-ROADMAP? Stop hook or validator where the runtime supports it при завершении Stage? Или никто?
- Сигнал «expected-vs-actual дельта» приходит — что заставляет записать в learnings? Или дельты исчезают в истории чата?

Если ответ «ничего / только дисциплина» — это **structural failure**, а не косметический. Попадает в Шаг 5 (leverage) как кандидат на systemic fix. Обычный fix — связка из трёх hook or validator where the runtime supports its or validators where the runtime supports them (user prompt handling / PostToolUse / Stop) или один compose skill для hot-keeping.
...
Пример: 5 сбоев из разряда «модель не читает `_ops/` в начале сессии» → один fresh-session instruction load hook or validator where the runtime supports it с context injection = класс закрыт. Не 5 prescriptions.
...
Один конкретный сигнал, по которому через N сессий видно, сработало ли. Пример: *«через 3 сессии файлы из `node_modules` перестают появляться в file edit calls»*.
...
Тип hook or validator where the runtime supports it'а, matcher, event. Детали — [claude-code-guardrails.md](claude-code-guardrails.md).
...
- Какое правило больше не служит своему backlink'у (PROJECT-ROADMAP сдвинулся, правило осталось)?
...
Один файл, который новая сессия читает первым. Какой skill вызывается на типичные триггеры. Какие hook or validator where the runtime supports its or validators where the runtime supports them сработают автоматически.
...
Для load-bearing prescriptions (hook or validator where the runtime supports it matcher, новое правило в AGENTS.md, новый skill trigger, смена ownership) не полагайся на гипотезу — проверь эмпирически. Subagent приходит с чистым context window и своей role framing.

- **A/B probe** — два Codex subagent'а на идентичной задаче: с prescription активным и без. Разница = real effect, не гипотеза.
...
- **Capability verification probe** — для Шага 2: Codex subagent с минимальным prompt'ом видит ли реально установленный hook or validator where the runtime supports it / skill / MCP в действии. Это честный тест на «реально установлено vs текстовое упоминание».
