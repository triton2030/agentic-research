...
Порядок предпочтения при проектировании защиты: **runtime (hooks/permissions) → local skill → instruction text → 1task-contract handoff → human checkpoint**. Prompt-level правила — последнее средство, не первое.
...
Примеры: `project-roadmap` (thinking workflow для durable bet), `1task-contract` (task-level контракт), TDD, security review.
