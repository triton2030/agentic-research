...
description: >
  ...
  domain grounding, or raw intent to `strategy-discussion`; route durable user
  truth to `user-truth` and task details to `task-contract`.
---
...
detail is needed, route to `task-contract`.
...
3. If the user intent is still raw, the Approach is not chosen, domain grounding
   is weak, or consequential branches remain, route to `strategy-discussion`
...
1. If the first user message is still a raw desire or competing approaches are
   unresolved, route to `strategy-discussion` before bootstrap.
...
4. Fill Goal and the top-level Stage-chain only as far as it is grounded. If
   ...
   branches to `strategy-discussion`; do not invent Stages 2-N.
5. Stop and return control. The next action re-enters through `before-work`.
...
- Does not invent domain expertise or choose hidden branches silently; use
  `strategy-discussion` when prerequisites, Stage order, or Approach are
  uncertain.
