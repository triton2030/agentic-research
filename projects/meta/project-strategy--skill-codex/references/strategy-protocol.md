...
Do not let the levels collapse together. The skill handle is `project-strategy`;
...
If this cannot be answered concretely, route to `1strategy-discussion` before
...
- Stage headings changed but `_ops/plans/phase-NN-*` is stale -> let
  `1task-contract` materialize needed phase folders lazily when task work enters
  that Stage.
...
- Stage order depends on domain prerequisites the model cannot ground -> route
  to `1strategy-discussion`.

## Minor Update Mode

If the request fits an existing Stage and does not change strategy truth, do not edit `PROJECT-ROADMAP.md`. Let `1task-contract` handle execution detail in `_ops/plans/**/task-*.md`.
