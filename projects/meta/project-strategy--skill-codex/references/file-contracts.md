...
`project-strategy` owns `_ops/PROJECT-ROADMAP.md` and `_ops/learnings.md`.
`user-truth` owns `_ops/INTERVIEW.md`. `1task-contract` owns task files and
...
- 1task-contract may create the needed phase folder lazily from the active Stage;
...
A task anchors to `PROJECT-ROADMAP.md#Stage N` or a relevant `INTERVIEW.md` section. It does not require a Step in strategy. If no Stage fits, route back to `project-strategy`.
