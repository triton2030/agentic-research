...
description: >
  ...
  to `1strategy-discussion` if the same goal could be solved by meaningfully
...
Stage exists. The level-1 contract lives with `project-strategy`.

Do not repair a weak roadmap by inventing task files. If no Stage fits, route
to `project-strategy`; if approach branches remain unresolved, route to
...
1. Choose the relevant Stage from `PROJECT-ROADMAP.md`; if no Stage fits, route to `project-strategy`.
2. If the task could be solved by meaningfully different approaches, or task
   ...
   `1strategy-discussion` before freezing scope.
