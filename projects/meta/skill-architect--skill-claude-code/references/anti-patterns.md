...
- **Ожидать, что `project-strategy` сам вызовется при каждом сигнале.** `project-strategy` — процедура, а не fairy. Её вызов должен быть триггернут механизмом (UserPromptSubmit hook на preference keywords, Stop hook на Stage completion), а не надеждой.
