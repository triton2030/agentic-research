...
description: >
  ...
  `1strategy-discussion`; route durable user truth to `user-truth`, missing task
  contract to `1task-contract`, missing strategy anchor to `project-strategy`.
  Skip typos, facts, review, pure strategy talk, and imminent write checks.
---

# Before Work

Объяви в начале одной строкой: «Использую `1before-work` — сверяю следующий ход со strategy».
...
action changes the roadmap, route to `project-strategy`. If it needs task scope
or substeps, route to `1task-contract`. If the right approach is still unclear,
route to `1strategy-discussion`.
...
5. Run a light frame check: if the intended action hides a new branch, risky
   framing, or materially different approach, route to `1strategy-discussion`.
6. Extract the execution lesson: what these sources change about how this work
   should be done now.
7. If the ask does not fit any Stage, route to `project-strategy`; if execution
   is non-trivial and no task contract exists, route to `1task-contract`.
...
- Does not update `PROJECT-ROADMAP.md`; route strategy drift to `project-strategy`.
- Does not create or close task files; route durable task scope to `1task-contract`.
- Does not run expert discussion or domain clarification; route unresolved
  branches to `1strategy-discussion`.
