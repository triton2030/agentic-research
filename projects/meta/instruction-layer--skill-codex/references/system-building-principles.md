...
- **Preference order fix-layers**: runtime guardrail (hook or validator where the runtime supports it/tool permission / approval policy/validator) → local skill → instruction text → `task-contract` handoff → human checkpoint. Prompt-level прескрипция — последнее средство, не первое.
...
|---|---|---|---|
| **Структурная** (always-on) | `instruction-layer` | Автоматически в каждой сессии | PreToolUse hook or validator where the runtime supports it блокирует file edit в `node_modules`; tool permission / approval policy `Bash(rm -rf *): deny`; validator на схему commit message |
...
**Правило эскалации:** если `task-contract` раз за разом пишет **одну и ту же** Must-not — это сигнал, что правило должно жить в hook or validator where the runtime supports it/validator/tool permission / approval policy, не в receipt. Эскалация обратно в `instruction-layer` — собственно, это один из входов этого скила.
