...
Рабочая папка по Codex-скиллу `1guide-subagents`.

## Что Это Закрывает

`1guide-subagents` — короткий guide по типовым провалам native Codex
...
- Upstream owners остаются upstream: `project-strategy`, `1instruction-layer` /
  `1repo-shape`, `1task-contract`.
- `1guide-subagents` не компенсирует отсутствующий план, не придумывает критерии
  и не превращается в mini-orchestrator.
- Если нужный judgment стратегический, а не исполнимый, ход уходит в
  `1strategy-discussion`, а не в запуск workers.
