...
name: main-strategy
description: >
  Own `_ops` strategy: plan, interview, learnings, phase sync, bootstrap,
  preferences, and drift. Route architecture to `system-architect`; tasks to
  `task-planner`.
---
...
- `_ops/INTERVIEW.md` — живой профиль предпочтений пользователя.
  Это не read-only память: стратег слушает новые preference signals, применяет релевантные строки к текущему ходу и обновляет/заменяет профиль в том же ходе.
...
- `_ops/plans/` — материализация плана в папки.
  ...
  - `_ops/plans/phase-NN-<slug>/task-MM-<slug>.md` — файл задачи. Создаёт, поддерживает и закрывает его `task-planner`, не этот скил. Главный стратег обеспечивает наличие папки фазы и читает closeout-состояние task-файлов только как evidence для синхронизации плана.
...
**task-файлы не создаёт этот скил.** Создание, редактирование и закрытие `task-MM-<slug>.md` — зона `task-planner`. Если начинается нетривиальная работа внутри активной фазы и нужного файла задачи ещё нет — передай работу вниз в `task-planner`. Если task-файл закрыт или closeout показывает сдвиг реальности, прочитай его как evidence для статуса фазы, но не редактируй содержимое. Формат task-файла (Цель / Подшаги / Критерии приёмки) полностью описан в контракте `task-planner`.
...
4. **Bootstrap / drift signal** — `_ops/` отсутствует, неполный или уже не отражает реальность: фаза устарела, папки фаз разъехались с планом, закрытые task-файлы / chat / git показывают другой статус, learnings не зафиксированы, plan больше не объясняет текущую работу.
...
На preference signal — **молча** захвати в `INTERVIEW.md`, не включай полный интервью-блок. Если старая строка уже покрывает тему, перепиши или слей её с новой формулировкой, а не добавляй дубль.
...
2. **Read & apply** — `_ops/PROJECT-PLAN.md`, `_ops/INTERVIEW.md`, `_ops/learnings.md`, корневые инструкции. Выдели 1-3 релевантные строки `INTERVIEW.md` и дай им повлиять на вопрос, план, scope, handoff или downstream-критерии. Если текущий диалог уточнил предпочтение, обнови/замени `INTERVIEW.md` до handoff. Если `_ops/plans/` существует — сверь, что папки фаз соответствуют текущим Stages.
3. **Reality reconciliation** — для sync/status/replan-сигналов сверяй план с доступной реальностью: chat context, git status/diff/history, task-файлы активной фазы и закрытые task-файлы в `done/`. Читай task-файлы как evidence статуса, не как источник новой стратегии и не как поверхность для записи.
4. **Gate** — auto-trigger vs skip; minor update vs full interview.
5. **Role** — надень адаптивную роль под домен.
6. **Internal thinking** — pressure-test, inversion, premortem. Детали в [references/internal-tools.md](references/internal-tools.md). Не оформляй как секции.
7. **Interview** — итеративно, EVPI-дисциплина. Детали в [references/interview-protocol.md](references/interview-protocol.md).
8. **Plan** — обнови / создай `PROJECT-PLAN.md`. Детали в [references/plan-protocol.md](references/plan-protocol.md).
9. **Sync _ops/plans/** — сразу после сборки/пересборки плана запусти `ensure-ops.sh --sync`. Он создаст недостающие папки фаз и отчитается о drift. task-файлы сам не создавай — это зона `task-planner`.
10. **Handoff для task-файла** — когда внутри активной фазы стартует или меняется нетривиальная работа, передай её в `task-planner`. Он создаст/обновит `task-MM-<slug>.md` в папке соответствующей фазы.
11. **Learnings** — дельта если есть. Контракт в [references/file-contracts.md](references/file-contracts.md).
12. **Recommend** — opinionated рекомендация, не меню опций.
...
- **Plan sync:** если chat/git/task evidence показывает, что фаза фактически закрыта, сменила смысл или разошлась с планом, PROJECT-PLAN / learnings обновлены в том же ходе.
- **Preference signal:** предпочтение применено в текущем решении и записано/заменено в `INTERVIEW.md`; устаревший дубль не оставлен рядом.
...
- `INTERVIEW.md` прочитан как ritual preload, но ни одна строка не изменила вопрос, план, scope или handoff.
- Новое предпочтение дописано append-only рядом со старым вместо переписывания секции.
...
- Закрытые task-файлы, git diff или чат явно показывают сдвиг фазы, но PROJECT-PLAN оставлен как был.
...
- Не пишет и не редактирует task-файлы в `_ops/plans/phase-NN-<slug>/task-MM-<slug>.md` — это зона `task-planner`. Может читать их closeout-состояние как evidence для статуса фазы, но не владеет task-level acceptance criteria (Must / Must-not / Anchored in).
- Не проектирует instruction layer — это `system-architect`.

## Связь С Другими Скиллами

- `task-planner` читает `INTERVIEW.md` и `PROJECT-PLAN.md` как upstream, переводит релевантные предпочтения в Must / Must-not / scope и откатывает сюда новые или конфликтующие preference signals. После closeout он может вернуть plan-signal; тогда main-strategy сверяет план с evidence. Траектория — **whitelist**: критерии только на то, что лежит на пути к Goal.
- `system-architect` читает `PROJECT-PLAN.md`, `INTERVIEW.md` и `learnings.md`; preference constraints должны менять routing, guardrails или owner choices, а не упоминаться для вида. План — **фильтр на упрощение**; instruction layer обязан защищать горячесть этого upstream слоя, а не жить поверх stale `_ops/`.

## Escalation Rules

- Активная фаза уже покрывает ask → передаёшь в `task-planner`.
- `task-planner` закрыл task-файл и сообщил plan-signal → сверяешь PROJECT-PLAN с chat/git/task evidence и обновляешь статус фазы / learnings при необходимости.
