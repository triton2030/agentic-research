...
## Работает downstream от `project-strategy`

- **Upstream**: `project-strategy` владеет PROJECT-PLAN + INTERVIEW + learnings. Без заполненных Goal и активного Stage архитектор блокируется и возвращается к стратегу.
- **Downstream**: `task-contract` получает handoff с указанием durable instruction surfaces как upstream. Если архитектор отработал хорошо, `task-contract` делает меньше работы — правила живут в fabric.

## Два уровня защиты

- **Структурная** (always-on) — `instruction-layer` через hooks, permissions, AGENTS.md, skill routing. Автоматически в каждой сессии.
- **Task-specific** (on-demand) — `task-contract` через acceptance criteria. Когда явно вызван.

Если `task-contract` раз за разом пишет одну Must-not для одного паттерна — правило должно жить в hook/validator, эскалация обратно в архитектора.
