...
`готово новое намерение` — 2026-08-31, редакция 2
(`work/v2/commander-intent.md`). Стадия `goal-context.md` пройдена целиком:
функция восстановлена по FAST, момент вызова и власть над локальным контрактом
уточнены владельцем, проверка мысленным удалением пройдена.

Цепь израсходованных состояний: `нужен новый commander's intent` →
`готово новое намерение`.

Разрешает один проход через раздел `refactor.md` «После нового намерения»:
передать только commander's intent исполнителю с чистым окном. Гарантия
clean-room зависит от разрешения владельца поднять субагента — в этой сессии
агент не поднимает их без прямой просьбы.
