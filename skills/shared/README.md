...
- `1folder-tree/portable/` — устройство базы знаний проекта как дерева папок:
  ...
  «в какой папке что хранить». Тело не менялось. Шаблон конвенций — `assets/`,
  парные примеры — `references/`; `platforms/codex/agents/openai.yaml` —
  только Codex UI metadata.
