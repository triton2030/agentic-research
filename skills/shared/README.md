...
- Семья авторинга — пять скилов по различимым моментам запуска, заменили пару
  ...
  - `1instruction-authoring/portable/` — создание и переписывание агентных
    инструкций проекта (v10, 2026-09-01). Функция — предотвратить критичный
    конфликт: доставить в точку работы правду, которую сильная модель по
    привычке нарушит и из этой точки не увидит. Форма v14: Уникальный контекст,
    Задача, Цель, Критерии принятия; тело — трёхшаговый роутер, единиц 5.
    Владеет `references/discovery.md` (обнаружение критичного вычитанием
    привычки агента из правды проекта, семь вопросов зоны, 17 единиц),
    `references/writing.md` (адресат обязательства, размещение, форма единицы,
    бюджет головы зоны, 19 единиц) и `agents/zone-reader.md` (веерное чтение
    ...
    Maintain-целью по KAOS, критерий — только контрмерой названному препятствию.
  ...
  - `1skill-creation/portable/` — создание, рефактор и кнопка запуска скилов
    одним пакетом (v16, 2026-09-01; предшественники и снапшоты —
    ...
    2026-08-31. Отбор инструкций — по Minimal Critical Specification (Cherns,
    ...
    намерению, а снятие называется владельцу в разборе. Бюджет пофайловый: до
    20 самостоятельных единиц, где единица — отдельная инструкция или
    ограничение; эти четыре раздела в счёт не входят при пределе в пять
    Критериев принятия на файл, а сумма тела и одного reference-файла
...
- `1document-system/portable/` — запись долгоживущего в документы проекта: где
  информация должна лежать, каким деловым типом называться и что в неё вообще
  попадает (v3, 2026-09-01). Один файл: Уникальный контекст, Задача, Цель, пять
  Критериев принятия и четыре единицы протокола. Вес намеренно перенесён в цель
  и критерии по доктрине `science/how-to-command-with-goal-not-procedure.md`;
  протокол несёт только то, чего цель выполнить не может — конвенцию пересказа,
  обязательство за пределами хода, проприетарное знание о чужих семьях
  артефактов и разделение переросшего документа. Влил в себя `1ia-audit`,
  снятый 2026-09-01 (история — `skills/1ia-audit/`); шесть маршрутов в
  `1smart-simple`, `1goal`, `1md-graph`, `1md-search` переписаны по смыслу.
