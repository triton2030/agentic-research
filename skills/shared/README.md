...
- `1folder-tree/portable/` — устройство базы знаний проекта как дерева папок:
  карта для владельца и источник правды для агентов (путь как знание,
  папка-вещь / папка-полка, файл как один вопрос, один факт — одно место).
  Установлен 2026-09-09 из внешнего пакета `kb-tree.skill` с переименованием
  по решению владельца; триггер — русские фразы «дерево папок»,
  «информационная архитектура», «куда положить файл», «как назовём папку»,
  «в какой папке что хранить»
  (`_ops/chat-recall/2026-09-09-165057-claude-code-ad6a2458.md#L16`).
  В тот же день тело переписано по четырём решениям владельца: проектные
  правила дерева живут в `AGENTS.md` корня базы, а не в `_конвенции.md`
  (`#L19`, шаблон конвенций снят); файлы внутри вещи делятся по длине и
  чистоте логического разделения, а не по фиксированному набору вопросов
  типа (`#L20`); деления файлов на «для агента» и «для владельца» нет,
  маркер `_` снят (`#L21`); в теле документа отсылка идёт к вещи-папке, а
