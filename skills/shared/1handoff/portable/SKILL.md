...
наступает на грабли этой сессии, не собирает заново модель предмета и не
надевает опровергнутую, — а полезное, что здесь помогало, знает заранее.
Слой агентного управления достаётся ей прибранным. Recall-ветка разрешена
по собственному gate.
...
- Consumer proof пройден: пять ответов выводимы из файла без чата
  (Механика 9).
- Решения и поправки владельца — только якорями на recall-записи
  (Механика 3).
- Каждый claim о состоянии работы несёт статус знания и последствие для
  преемника (Механика 4).
...
- Совет следующему агенту несёт наблюдения с привязкой к файлам и типам
  работ — не рекомендации «как улучшить» и не абстрактные уроки; урок,
  который должен сработать позже, предъявлен установленным бортиком, а не
  строкой совета (Механики 5 и 7).
...
не замечаются вовсе — опыт сессии умирает вместе с ней; сменившаяся модель
предмета не восстанавливается — преемник собирает её заново из тех же файлов
и приходит к тому же заблуждению.
...
2. **Уборка за собой.** Просмотри слой агентного управления целиком: план и
   ...
   строки инструкций и доков, устаревшие после работы этой сессии · бортики
   Механик 5 и 7 — ставятся здесь же и по этой же границе · git —
...
4. **Ядро:** где работа реально остановилась (включая незавершённое и
   ...
   несёт два ярлыка: статус знания — «чем проверено» (команда, тест, diff)
   либо `непроверено` — и последствие для преемника: `нет` · `принято как
   допущение` · `блокер` · `переосмыслить`. Один ярлык вместо двух — главный
   сбой жанра: проверенное неотличимо от непроверенного, а что с этим делать,
   преемник достраивает сам.
...
5. **Incident cell** — только когда новое чтение, tool output или поправка
   ...
   **цена** (wall time по timestamps либо `unknown`) → **почему поздно** (либо
   `unknown`) → **предотвращение**: поставленный бортик — что изменено и где
   (шапка скрипта, строка инструкции, тело скила, проверка) — либо явная
   причина, по которой бортик не ставится. Фраза, адресованная будущему
   читателю, предотвращением не является. Безвредное расхождение ожиданий —
   не инцидент; telemetry не выдумывается.
6. **Модель местности** — что преемник должен думать о предмете прежде, чем
   начнёт. **Рабочая модель** в 3–5 строк: как устроено то, в чём работать, в
   том виде, к которому сессия пришла. **Ловушки**, 2–4 блока: `что выглядит
   очевидным (файл, вывод, имя) → какую модель это порождает → чем опровергнуто
   (команда, вывод, поправка владельца) → что верно`. Ловушка пишется, только
   если породившая её местность осталась на месте: исправленный файл ловушки
   больше не несёт. Сюрприз без цены и без последствия для преемника не
   пишется — это дневник. Единица здесь — адресуемое убеждение, не глава
   истории: хронологический рассказ «как я думал» не пишется. Объект раздела —
   убеждение; событие, сменившее маршрут, — Механика 5, закрытый способ —
   rejected route Механики 4.
7. **Совет следующему агенту** — мета-анализ собственной сессии, содержательно
   ...
   наблюдения. Наблюдение, которое должно сработать позже, либо уже
   встречалось в советах прежних хендофов (`grep` по `_ops/handoffs/`), в
   совет не идёт: оно уходит бортиком по Механике 2 — в файл, читаемый в
   момент решения, — а сюда попадает строкой «что установлено».
8. **Файл:** `mkdir -p _ops/handoffs && date +"%Y-%m-%d-%H%M%S"` →
   ...
   причиной и сверь HEAD-якорь с `git log`. Каркас — минимум, не шаблон:
   `## Модель местности` · `## Где мы сейчас` · `## Следующий шаг` ·
   `## Совет следующему агенту` · `## Якоря`. Veto, блокер и всё с ярлыком
   `непроверено` стоят в преамбуле либо в якорях: середина файла —
   слабейшая зона окна читателя. В якорях: HEAD-коммит на момент хендофа,
   исходы recall-ветки и уборки, путь предыдущего полученного хендофа, если
   он был.
9. **Consumer proof (жёсткий переход).** Перечитай файл как свежий агент с
   repo и путём, но без чата: следующий шаг и почему · pending/blocker/veto ·
   какой тупик не повторять · какое очевидное прочтение местности неверно ·
   где live truth. Невыводимо → добавь дельту; блок
