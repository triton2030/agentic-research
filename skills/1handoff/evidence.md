...
Первая документированная проба потребителя на реальном пакете: свежий агент
Claude Opus 5 получил только путь к
`mavo3/_ops/handoffs/2026-09-18-163932-claude-fable-5-1.md` (скил версии
2026-08-29). Первое действие названо верно за ~10 минут и 6 файлов; без ответа
остались: основание рекомендации в Next Step, статус вопроса владельцу, работа
при молчании владельца, расхождение файла задачи с пакетом. Корпус: из 22
пакетов текущей формы 18 сами дописали состояние рабочего дерева, 9 — в
середине файла. Протокол и цифры — `draft-2026-09-18/probe-2026-09-18.md`.

Путь: черновик → три проверяющих `1skill-creation` в свежих окнах (14 находок,
решения — `reviews-2026-09-18.md`) → правка → вторая волна проверяющего
траектории по затронутым единицам → установка в оба tracked owner и обе живые
копии.

Поведение под новым текстом — `candidate` до первого реального хендофа и
повторной пробы тем же способом.
