...
- Не бери полномочий, которых у этой сессии не было. Приведение в порядок
  того, что она сама сделала ложным, в них входит, даже если исходная задача
  этих файлов не касалась.
...
- Имена, списки и примеры в этом скиле ориентируют и не ограничивают состав
  работы; запреты, гейты и перечисленные исходы стадий, обязательные разделы
  пакета и словари ярлыков, наоборот, ограничивают её.

## Стадии

Работай стадиями по порядку.
Читай ровно один файл стадии за раз.
Не открывай следующий, пока текущая стадия не закрыта своим выходом.
...
5. Когда файл пакета лежит на диске, прочитай `references/read-as-next-agent.md`.
   Если пакет заблокирован, эту стадию пропусти: перечитывать нечего, и её
   исход — `не выполнялся`.
6. Перед тем как путь или блокер уйдёт в чат, прочитай
   `references/report-and-stop.md`.
