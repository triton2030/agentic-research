...
REPO = WORK.parents[3]
# Живой owner, не исторический пакет: тесты проверяют то, что установится.
# Проверка переехала из 1plan-map в 1planning; эти тесты держат прежнюю форму.
CHECKER_PATH = REPO / "skills" / "shared" / "1planning" / "portable" / "scripts" / "check_plans.py"
LEGACY_CHECKER_PATH = REPO / "skills" / "1planning" / "work" / "implementation-2026-09-08" / "packages" / "1plan-map" / "scripts" / "check_plans.py"
...
class CheckerTests(unittest.TestCase):
    ...
    def test_section_order_and_checked_report_evidence(self) -> None:
        holder, fixture = self.make_fixture()
        with holder:
            text = fixture.task.read_text(encoding="utf-8").replace("## Основания", "## Зачем", 1)
    ...
    def test_snapshot_changes_with_intent_and_grounds_but_not_derived_or_state(self) -> None:
        holder, fixture = self.make_fixture()
        with holder:
    ...
    def test_stale_snapshot_is_data_error(self) -> None:
        holder, fixture = self.make_fixture()
        with holder:
            (Path(holder.name) / "docs" / "decision.md").write_text("changed\n", encoding="utf-8")
            report = check_plans.validate_root(Path(holder.name))
    ...
    def _write_legacy_pair(self, fixture: Fixture) -> str:
        """Rewrite the fixture with the pre-2026-09-09 names and a matching snapshot."""
        root = fixture.root
        epic = fixture.epic_text().replace("основания: [", "канон: [", 1).replace("## Основания", "## Канон", 1)
        fixture.epic.write_text(epic, encoding="utf-8")
        digest, report = check_plans.snapshot_path(root, fixture.epic.relative_to(root).as_posix())
        assert digest and report.ok, [issue.message for issue in report.issues]
        task = fixture.task_text(snapshot=digest).replace("## Основания", "## Канон", 1)
        fixture.task.write_text(task, encoding="utf-8")
        return epic

    def test_legacy_canon_names_still_validate(self) -> None:
        holder, fixture = self.make_fixture()
        with holder:
            root = Path(holder.name)
            epic = self._write_legacy_pair(fixture)
            report = check_plans.validate_root(root)
            self.assertTrue(report.ok, _messages(report))
            missing = epic.replace("канон: [\"docs/decision.md\"]\n", "")
            fixture.epic.write_text(missing, encoding="utf-8")
            self.assertIn("missing required field 'основания'", _messages(check_plans.validate_root(root)))

    def test_legacy_epic_snapshot_matches_previous_checker(self) -> None:
        spec = importlib.util.spec_from_file_location("legacy_check_plans", LEGACY_CHECKER_PATH)
        assert spec and spec.loader
        ...
        with holder:
            root = Path(holder.name)
            self._write_legacy_pair(fixture)
            rel = fixture.epic.relative_to(root).as_posix()
            new_digest, new_report = check_plans.snapshot_path(root, rel)
            old_digest, old_report = legacy.snapshot_path(root, rel)
            self.assertTrue(new_report.ok and old_report.ok)
            self.assertEqual(new_digest, old_digest)
