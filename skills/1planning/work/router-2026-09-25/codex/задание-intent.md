# Проверка скила `1planning` — роль intent
...
Проверь смысл разделов черновика по роли `/Users/triton/.claude/skills/1skill-creation/agents/check-intent.md`: какую дельту к знанию сильного агента несёт Контекст, отличается ли поведение с этим скилом от поведения сильного агента без него, держит ли Цель выбор при конфликте способов, закрывает ли каждый Критерий ошибку, ещё допустимую Контекстом, Задачей и Целью вместе. Что здесь не выполнено и чем это видно?
