# Проверка скила `1planning` — роль trajectory
...
Проверь применение по роли `/Users/triton/.claude/skills/1skill-creation/agents/check-trajectory.md`. Пройди реалистичные случаи: владелец говорит «давай запланируем следующую работу»; агент собирает роутер; чистый агент исполняет задачу по роутеру; агент продолжает задачу прежней формы (живой пример — `/Users/triton/Documents/My_projects/mavo3/_ops/plans/эпики/Документация для разработки/task - Перенос модели каталога.md`); закрытие задачи; ответ владельца меняет продолжение. Для каждого: доступен ли нужный текст до выбора, который он направляет. Сам проведи безопасные пробы в копии: тесты механизма из `skills/1planning/work/router-2026-09-25/` и `check_plans.py --root` на `/tmp/planning-probe` и на `/Users/triton/Documents/My_projects/mavo3` без `--sync`; по каждой пробе — команда и exit code. Сверь с evidence автора. Что здесь не выполнено и чем это видно?
