...
| Один ответственный агент пишет файл, помощники возвращают evidence | уточнить | протокол 6 |
...
- **Бюджет около 20:** default — файл пересказывает весь корпус → механизм —
  surprise-отбор и явное число → вред — занижение счёта или искусственный split
  → цена — запрет склеивания и `неделимо — <причина>` без реальной оси.
...
- **Пять инвариантов:** default — scope creep после нового evidence → механизм
  — stop при изменении принятого смысла → вред — ложный stop на перефразировании
  → цена — редакционная смена слов разрывом не считается.
