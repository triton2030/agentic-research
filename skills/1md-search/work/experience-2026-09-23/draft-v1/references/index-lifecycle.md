...
Confirm — долгая запись: ход работы идёт в поток ошибок, итоговый JSON
приходит в конце. Обрезанный вывод (`head`, закрытый конвейер) обрывает
сам процесс, и индекс остаётся обновлённым частично, поэтому сохраняй вывод
целиком в файл и читай оттуда.
