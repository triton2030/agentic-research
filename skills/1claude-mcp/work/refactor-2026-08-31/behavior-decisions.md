...
- **Подготовка:** готовый brief и реальный project/worktree `cwd`; после запуска
  нет полезной независимой работы Codex.
...
- **Проверка:** один returned packet либо один diagnostic отказа и ровно одно
...
- **Проверка:** возвращены тот же `session_id`, `accepted_op`, `state`, `cursor`
  и terminal metadata; follow-up с ожидаемым ответом заканчивается только на
  terminal Opus result, а после stop состояние закрыто или очистка наблюдаема.
...
- Runtime-механика принята только для точных tool/state semantics, которые не
  выводятся из commander's intent и наблюдаемы через callable schema и typed
  result.
