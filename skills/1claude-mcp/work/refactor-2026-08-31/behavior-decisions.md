...
## Поведение, выводимое из задачи и цели

Из `Уникального Контекста`, задачи и цели уже выводятся независимая роль Opus,
полное владение исследованием/review, законченное заключение, локальная проверка
и владение Codex финальным ответом. Отдельный протокол мышления для этого
поведения не допускается.

## Owner evidence — дословно
...
`_ops/chat-recall/2026-08-31-212001-Codex-01a0589c.md:18,20-21`.
...
- Runtime-механика принята только для точных tool/state semantics, которые не
  ...
  result.
- Прежний дословный runtime `owner-protocol.md` снят по более позднему явному
  выбору владельца: три смысловые секции теперь несут роль, работу и end state;
  цитаты остаются здесь как provenance, но не перечитываются при каждом вызове.
