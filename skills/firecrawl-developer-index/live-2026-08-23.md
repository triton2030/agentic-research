...
description: Ищет в первичном источнике — README, доки, issues, merged PR — что уже написано и как оно себя ведёт. Вызывай перед тем, как писать любой новый код сверх мелкой правки: сначала проверь, что готового не существует. Вызывай и когда собираешься назвать по памяти — себе или в ответ владельцу — версию, дефолт, сигнатуру, причину ошибки или то, что баг уже починен. Признак момента — сама уверенность при неоткрытом первичном источнике: чем меньше ощущается вопрос, тем вернее момент, и владелец при этом ни о чём не спрашивал. Не вызывай для сравнений и мнений; версия или дефолт самого CLI-инструмента — `1cli-tools`, оформление уже написанного кода — `1readable-code`.
---
