...
description: Knows what your training doesn't — a current index of GitHub issues, merged PRs, READMEs, and docs, built for coding agents. One call returns exact passages with links, so you quote instead of guessing what exists, what a tool or platform can do now, how it behaves, and what was fixed.
---
...
- HTTP: **`GET|POST https://api.firecrawl.dev/v2/search/developer`**
  ...
  Send `Authorization: Bearer <key>` by default; a search costs 2 credits per 10 results. Keyless requests share a small daily allowance per IP and can be refused (`429`, or "your IP address looks suspicious"), so use them only as a fallback when no key is available or credits run out. If `$FIRECRAWL_API_KEY` is unset, the key lives in the CLI credential store: `firecrawl view-config` reports auth status and the config directory, and the `apiKey` field of `credentials.json` there is the value. The CLI reads that store itself, so `firecrawl developer` needs no key passed.
...
- **Platform or product capability** ("can Vercel do this now", "did this tool change what it is for") → search the index in plain words, then read the vendor's current docs or changelog with `firecrawl_scrape` or `firecrawl_search`. Features and positioning move faster than memory, and `web:` blog hits only point to the source.
