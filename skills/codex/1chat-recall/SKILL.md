...
Для каждой цитаты, прошедшей usefulness gate, `context-note` обязателен. Он
содержит только surprise-дельту: то, что нельзя разумно вывести из цитаты вместе
с `type/topic`. Самодостаточность тезиса не делает его бесконтекстным: назови
внешний referent, scope, прежнее состояние или сцену решения, потерянные при
изоляции. Повтор, перефразирование или подтверждение уже понятного запрещены.
Если честную неповторяющую дельту нельзя назвать, перечитай соседний source
context; не заполняй поле пересказом.
...
python3 "$CAPTURE" \
  ...
  --context-note "<недостающее внешнее обстоятельство, не пересказ цитаты>" \
...
Перед запуском capture назови для каждой quote одно внешнее обстоятельство,
которое исчезнет без соседних сообщений: referent, scope, прежнее состояние или
сцену решения. Для `--kind quote` флаг
`--context-note "<одно короткое пояснение агента>"` обязателен. Заметка хранится
inline, явно остаётся недословной и показывается через `show`, но не участвует в
record ID, BM25 или dense ranking. Проверка против повтора: мысленно закрой
цитату — заметка должна только локализовать её применение, а не позволять
восстановить сам тезис. Не добавляй новый вывод, мотивацию, URL, путь либо ссылку
на transcript/другую запись. Ограничение — 300 символов; у `selection` поле
допустимо только для такой дельты, а `--kind note` с `--context-note`
недопустим. Context note никогда не делает слабую цитату достойной записи:
сначала usefulness gate.
...
- Если ответ остановился на quote dump, top hit объявлен текущим без gate,
  owner-цитата осталась без `context-note` либо заметка повторяет цитату, shaping
  не сработал: вернись к последней непройденной проверке, а не расширяй output.
...
Дизайн-гипотеза датирована 2026-08-05 для GPT-5.6. Смена рабочего model set или
