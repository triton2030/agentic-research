...
Четыре приёма попадания в нужную запись:

- **Корень, а не термин.** Владелец говорит своими словами: бери 2–3
  синонимических корня (`автоном|самостоятельн`), не профессиональный термин
  и не фразу целиком — она не повторится дословно.
- **Нет слова — заходи типом.** `type: предпочтение`, `обо-мне`,
  `правило-кандидат` — узкие классы, читаются целиком: класс вопроса заменяет
  угаданную формулировку.
- **Имя скила, файла или даты ищи в `context-note`.** Там референты названы
  устойчиво и тогда, когда владелец сказал «этот скил», — покрытие шире, чем
  у его собственной лексики.
- **Пустой ответ — не «владелец не говорил».** Смени корень или тип; полный
  проход по корпусу дёшев, сужать ради скорости незачем. Route исчерпан —
