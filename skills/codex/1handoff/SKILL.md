...
- Следующий агент, у которого есть только этот репозиторий и путь к пакету,
  уверенно начинает верное следующее действие и знает, где записано то, что
  останется сделать после него; не повторяет ошибок, за которые эта сессия уже
  заплатила временем, и не собирает заново ту модель предмета, которую эта
  сессия уже опровергла.
...
- Не проверяй и не улучшай то состояние проекта, которое эта сессия не сделала
  ...
  ревизии проекта. Заказ самой этой сессии под эту границу не попадает: он
  её собственное состояние, и его владелец приводится в порядок всегда.
...
Внутри этих границ выполни пять действий по порядку.
...
4. Дай заказу этой сессии живого владельца. Пакет им не становится: он держит
   один шаг, а заказ переживает много шагов. Если файл задачи есть, сверь его
   статус и счёт подзадач с тем, что сессия сделала: этот файл читают
   планировочные Bases, и лгущий статус — тоже ложь слоя управления.
   Расхождение исправь через `1plan-task`; если границы этой части не дают —
   назови его вместе с последствием. Если файла нет, а заказ ещё не исчерпан,
   заведи его через `1plan-task` там, где планирование уже живёт и заказ
   допущен по контракту `1planning` или утверждённым эпиком; допуска или эпика
   нет — назови заказ незакрытым, дай адрес слов владельца о нём в корпусе
   `1chat-recall` либо блокер `no recall address` из части 1 (вернись туда,
   если нужда открылась только здесь) и последствие: без владельца следующий
   агент после первого действия остаётся без работы или выдумывает её мимо
   veto. Если ближайшее действие исчерпывает заказ, подтверди это прямо.
   Владелец заказа записывается в строку остатка в пакете; если её значение —
   блокер, назови его ещё и в зоне сразу после преамбулы. Сама правка файла
   задачи остаётся названным изменением в значении `cleanup`.
5. Если один и тот же урок оплачен уже более одного раза — по записанной истории
...
- Каждый вопрос владельцу, от ответа на который зависит продолжение, проходит
  в пакет вместе со своим состоянием: задан или не задан, отвечен или нет,
  адрес ответа, если он есть, и на каком допущении или независимой работе
  сессия шла, пока ответа не было. Veto «не делай без слова владельца» этого
  состояния не несёт: следующий агент либо спрашивает второй раз, либо ждёт
  ответа, которого владелец не собирался давать.
- Незакоммиченные правки, фоновые процессы и временные папки, которые
  переживут эту сессию, проходят в пакет с одной из трёх пометок: намеренно,
  снять можно, чужое или неизвестного происхождения. Номер коммита этого не
  говорит: по нему следующий агент не отличит начатую работу от мусора, а
  чужое без своей пометки он снесёт как мусор.
...
3. Каждый veto, каждый блокер и каждое непроверённое состояние, которые меняют
   ...
   Вопрос владельцу, без ответа на который работа стоит, — тоже туда вместе с
   последствием; раздел `## Questions to the Owner` держит состояние всех
   вопросов, а не заменяет эту зону. Середина файла — это та зона, где
   читатель перестаёт видеть написанное.
...
7. Причине в разделе `## Next Step` и рекомендации, если она в разделе всё же
   есть, поставь только ярлык `knowledge`: адрес слов владельца, файла или
   наблюдения либо `unverified`. Ярлык `consequence` им не ставится. Без адреса
   следующий агент подаёт владельцу совет, оснований которого не знает.
8. Раздел или строку скелета, помеченные маркером `<Гейт: …>`, оставляй в файле
...
9. Отсутствие тупика, инцидента или ловушки подтверждай прямо, а не выдумывай их
   ради того, чтобы раздел не выглядел пустым.
10. Если запись файла прервалась и довести его до заполненного состояния
   невозможно, удали этот неполный файл.
11. Тогда назови точный блокер записи, часть 5 пропусти — перечитывать нечего, и
...
- называет верное следующее действие, начинает его и объясняет, почему верным
  является именно оно;
- находит живой файл задачи с остатком работы, не противоречащий пакету или
  с названным расхождением, либо прямое подтверждение, что остатка нет, либо
  названный блокер с адресом заказа;
- отличает датированное утверждение пакета от живой правды проекта;
- находит evidence для каждого материального утверждения о состоянии и
  адрес у причины и рекомендаций в `## Next Step`;
...
- про каждый вопрос владельцу, от которого зависит продолжение, знает, задан
  ли он, отвечен ли и на чём сессия шла без ответа;
- отличает намеренные незакоммиченные правки, чужие правки и живые фоновые
  процессы от того, что снять можно;
