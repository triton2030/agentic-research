...
fallback пустого поиска. Carrier, similarity и metadata сами не доказывают owner
authorship. Helper подтверждает provenance сообщения, timestamp и source
address, но не доказывает авторство каждого фрагмента внутри вставленного
текста.
...
2. Только после явного owner request для named session/source через runtime
   `chat_recall.py --help` докажи literal text, provenance сообщения и source
   address; затем отдели owner-authored span по deletion-only правилу Capture.
   Иначе не читай transcript и верни gap.
