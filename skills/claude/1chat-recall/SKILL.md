...
`support` — сырая близость ближайшей записи корпуса, снятая до слияния каналов;
RRF-ранг такого смысла не несёт, и десятка выдаётся всегда. `unsupported`
означает, что материала по предмету в корпусе нет: не пересказывай выдачу, а
назови gap и либо переспроси корпус лексикой самой темы, либо закончи ветку
`abstain`. `weak` — полоса неопределённости: читай, но позицию на одном таком
запросе не строй. Вердикт говорит о наличии материала, не о правоте выдачи.
