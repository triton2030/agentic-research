...
Частый читатель — не главный агент, а read-only субагент из свежего окна:
у него нет истории чата, и корпус — единственный канал к намерению владельца.
Корпус при этом систематически свежее нормативных файлов и может им прямо
противоречить. Нашёл расхождение — назови оба адреса вызывающему и оставь
след; молча выбирать сторону нельзя, чинить норматив из чужого окна тоже.

Как устроен корпус — путь поиска выбирай из этого сам:
...
- Один файл — один разговор, имя `<дата>-<время>-<агент>-<session8>.md`.
  ...
  вывод всегда ограничивай: широкий греп вываливает сотни килобайт и съедает
  четверть окна одной командой.
- Запись — одна строка:
  `* <timestamp> — "<слова владельца>" — type: … | topic: … | context-note: …`
  В кавычках речь владельца дословно; `context-note` написан агентом и
  называет сцену устойчивыми именами (скил, файл, дата) — их в самой цитате
  может не быть.
- Frontmatter файла перечисляет его `types:` и `topics:` — инвентарь
  разговора без чтения тела.
- Типы: `решение` · `коррекция` · `критерий` · `предпочтение` · `идея` ·
  `правило-кандидат` · `обо-мне` · `факт`. Тип говорит, чем реплика была:
  `идея` — размышление вслух, не принятый курс; `решение` — курс.

Пустой ответ — не «владелец не говорил»: смени корень слова или зайди типом.
Route исчерпан — [reading-the-log](references/reading-the-log.md).
