---
name: 1chat-recall
description: Владелец слов пользователя. ЗАПИСЬ автоматична и обязательна, без просьбы сохранить, в том же ходе: владелец сказал стоящее сохранения — решение, коррекцию («нет, по-другому», «мне не нравится, когда…»), red line, предпочтение, критерий, важное правило или число, сырую идею на будущее, факт о себе или «потому что»-хвост разовой команды — зафиксируй его формулировку в `_ops/chat-recall/`. Это НЕ заменяется памятью: память хранит твой вывод, skill хранит его слова; нужны обе записи, не выбирай одну. ЧТЕНИЕ обязательно, когда результат опирается на сказанное: «что я говорил/выбрал», «я уже отвечал», точные слова, числа, условия, сверка плана после compaction — skill читает transcript вместо ненадёжного контекста.
allowed-tools: Bash(${CLAUDE_SKILL_DIR}/scripts/chat_recall.py *), Bash(${CLAUDE_SKILL_DIR}/scripts/chat_capture.py *)
---

# Chat recall

Слова владельца точны только в момент, когда прозвучали: контекст выцветает и
пересказывается. Два скрипта держат их точными: capture копит датированную руду
цитат, recall читает transcript текущего разговора.

## Запись — автоматически, в момент, когда прозвучало

Триггер — не просьба сохранить, а сам факт: владелец сказал то, что стоит
сохранить. Решение, коррекция твоего хода, red line, предпочтение, критерий
готовности, ограничение, важное число, факт о том, как владелец работает, сырая
идея на будущее («не забыть бы, но пока рано»). Отдельно лови durable-хвост
императива: в «сделай X, потому что в будущем Y» команда разовая и не
записывается, а хвост про Y — записывается. Пиши сразу в том же ходе, одной
командой, и продолжай работу — это не отдельная задача и не повод для вопроса.

```bash
"${CLAUDE_SKILL_DIR}/scripts/chat_capture.py" --quote "<его слова, слегка сокращённые>" --type решение --topic 1codex --model claude-fable-5 --project "$PWD"
```

- `--type` — из закрытого списка: `решение`, `коррекция`, `предпочтение`,
  `идея`, `критерий`, `правило-кандидат`, `обо-мне`, `факт`. Новых не выдумывай.
  Возможная инструкция потом ищется по `коррекция|правило-кандидат|предпочтение`.
- `--topic` — существующий handle: имя скила, проекта, папки, подсистемы
  (`1codex`, `md-tools`). Слова владельца о себе и своём способе работать —
  постоянная тема `мой-workflow`. Нет очевидного handle — возьми имя проекта.
- `--model` — свой model id, если знаешь.

Скрипт сам ведёт файл этого разговора
`_ops/chat-recall/<дата>-<время>-<агент>-<session8>.md` (session id своего
агента из env; не нашёл — откажет: передай `--session`, общий файл смешал бы
разговоры) и обновляет инвентарь `types`/`topics` в его шапке.

Что попадает в `--quote`:

- **Его слова, не твой пересказ.** Никакого «пользователь хочет, чтобы…»,
  никакой твоей интерпретации, никаких выводов о причине.
- **Слегка сокращённые.** Убери «ну», «короче», «как бы», самоперебивы,
  повторы одной мысли другими словами. Остальное оставь дословно —
  формулировку, порядок мысли, его лексику.
- **Смысловое ядро целиком.** Сокращение — чистка речи, не выжимка: если из-за
  него потерялось условие, оговорка или «но», сократил неверно. Лучше длиннее и
  точно, чем коротко и приблизительно.
- **Одна мысль — одна запись.** Два разных устойчивых тезиса в одной реплике —
  две команды.

Только текущий разговор: записывай реплики этой session; не переноси цитаты из
чужих recall-файлов, чужих транскриптов или пересказов. Субагент не записывает
вовсе: в его транскрипте «пользователь» — вызывающий агент, а не владелец;
цитаты владельца фиксирует только главный агент разговора.

Не записывай: разовые операционные указания текущей задачи («поправь этот
файл», «запусти тесты»), уточняющие вопросы, согласия вроде «да, давай»,
credentials и токены.

## Статус лога: руда с датами, не канон

Лог односторонний — слова владельца без развязки разговора: сказанное могло
быть позже молча отменено. Поэтому:

- Не читай `_ops/chat-recall/` перед решениями по своей инициативе. Открывай
  только по явной команде владельца («что я говорил про…») или в его
  harvest-задаче (повторяющиеся коррекции → правка инструкций, автоматизмы).
- Подтверждённое правило в момент подтверждения уходит в `AGENTS.md` / память;
  лог хранит цитату-источник. Две записи, не одна.
- Поиск: сперва инвентари шапок (`types`/`topics`), тела — только у нужных
  файлов. Точные выборки: `rg 'topic: 1codex' _ops/chat-recall/`; кандидаты в
  инструкции — `rg 'type: (коррекция|правило-кандидат|предпочтение)'`.

## Чтение — когда результат опирается на сказанное

```bash
"${CLAUDE_SKILL_DIR}/scripts/chat_recall.py" --session-id "${CLAUDE_CODE_SESSION_ID}"
```

Возвращает последние 10 реплик владельца из активной ветки текущей session с
датой и временем, текущий turn исключён. Нужна полнота — повтори с `--all`.

Используй только прямые сообщения и подтверждённые ответы `AskUserQuestion`.
Формулировка вопроса и option label принадлежат Claude; считай их
зафиксированным выбором, а не дословной репликой владельца. Image content не
извлекается и помечается явно.

`session` в шапке recall-файла — ключ к полному транскрипту того разговора
(`~/.claude/projects/<проект>/<session>.jsonl`), пока его не удалил retention.
Чужой транскрипт открывай только по явной команде владельца.

Если helper завершился ошибкой, скажи, что recall не подтверждён. Не угадывай
по памяти и не открывай другую session по своей инициативе.
