...
- `--topic` — тема тезиса, не проект, в котором он прозвучал: реплика про канон
  посреди работы над презентацией — это `topic` канона. Бери существующий
  handle: имя скила, проекта, папки, подсистемы (`1codex`, `md-tools`); перед
  новым handle глянь занятые
  (`chat_digest.py <корень>/_ops/chat-recall`) — дубль в другом регистре ломает
  поиск. Слова владельца о себе и своём способе работать — постоянная тема
  `мой-workflow`. Нет очевидного handle — возьми имя проекта.
