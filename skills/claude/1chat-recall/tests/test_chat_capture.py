...
class ChatCaptureTests(unittest.TestCase):
    ...
    def test_json_receipt_addresses_the_saved_raw_record(self) -> None:
        ...
        self.assertEqual(receipt["line_anchor"], f"{holder.name}#L{line_number}")
        self.assertEqual(receipt["anchor"], f"{holder.name}#{receipt['record_id']}")
    ...
    def test_earlier_source_quote_keeps_published_session_path_and_date(self) -> None:
        ...
        files = self.recall_files()
        self.assertEqual(len(files), 1)
        local_earlier = datetime.fromisoformat(later).astimezone()
        self.assertTrue(
    ...
    def test_append_extends_legacy_holder_without_touching_old_lines(self) -> None:
        ...
        legacy_text = re.sub(r" \| source-ref: [^\n| ]+", "", legacy_text)
    ...
    def test_selection_and_note_use_only_exact_record_metadata(self) -> None:
        ...
        self.assertIn("source-ref: timestamp-", text)
    ...
    def test_new_topic_extends_empty_map(self) -> None:
        topic_map = self.root / "_ops" / "chat-recall" / "topics.md"
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("`html-artifacts`", topic_map.read_text(encoding="utf-8"))
        self.assertEqual(len(self.recall_files()), 1)

    def test_first_capture_creates_map_and_holder(self) -> None:
        self.run_capture("Первая реплика", "решение", "first", env=self.claude_env(),
                         new_topic="Первая тема", ensure_topic_map=False)
        self.assertEqual(len(self.recall_files()), 1)
        self.assertIn("first", (self.root / CHAT_CAPTURE.TOPIC_MAP).read_text())

    def test_first_capture_without_boundary_is_actionable(self) -> None:
        result = self.run_capture("Первая реплика", "решение", "first", env=self.claude_env(),
                                  ensure_topic_map=False, expect_ok=False)
        self.assertEqual(result.returncode, 2)
        self.assertIn("--new-topic", result.stderr)
        self.assertEqual(self.recall_files(), [])

    def test_first_capture_rolls_back_map_after_holder_failure(self) -> None:
        argv = [str(SCRIPT), "--quote", "Первая реплика", "--type", "решение",
                "--topic", "first", "--new-topic", "Первая тема",
                "--context-note", DEFAULT_CONTEXT_NOTE,
                "--session-context", DEFAULT_SESSION_CONTEXT,
                "--source-timestamp", DEFAULT_SOURCE_TIMESTAMP,
                "--supersedes-none", "--agent", "claude",
                "--project", str(self.root), "--session", self.session]
        real_create = CHAT_CAPTURE.create_file
        def fail_after_write(*args, **kwargs):
            real_create(*args, **kwargs)
            raise OSError("simulated holder failure")
        with mock.patch.object(sys, "argv", argv), mock.patch.object(
            CHAT_CAPTURE, "create_file", side_effect=fail_after_write
        ):
            self.assertEqual(CHAT_CAPTURE.main(), 2)
        self.assertFalse((self.root / CHAT_CAPTURE.TOPIC_MAP).exists())
        self.assertEqual(self.recall_files(), [])

    def test_unresolved_supersession_is_saved_and_exclusive(self) -> None:
        self.write_topic_map("- `first` — Первая тема")
        argv = [str(SCRIPT), "--quote", "Теперь иначе", "--type", "решение",
                "--topic", "first", "--context-note", DEFAULT_CONTEXT_NOTE,
                "--session-context", DEFAULT_SESSION_CONTEXT,
                "--source-timestamp", DEFAULT_SOURCE_TIMESTAMP,
                "--supersedes-unresolved", "Поиск старого варианта не дал цитаты",
                "--agent", "claude", "--project", str(self.root), "--session", self.session]
        with mock.patch.object(sys, "argv", argv):
            self.assertEqual(CHAT_CAPTURE.main(), 0)
        self.assertIn("supersedes-unresolved:", self.recall_files()[0].read_text())
        for option in (["--supersedes-none"], ["--supersedes", "old.md:1"], ["--contested", "old.md:1"]):
            with mock.patch.object(sys, "argv", argv + option):
                self.assertEqual(CHAT_CAPTURE.main(), 2)

    def test_new_topic_receipt_failure_restores_map_and_holder(self) -> None:
        ...
        with mock.patch.object(sys, "argv", argv), mock.patch.object(
            CHAT_CAPTURE, "record_receipt", side_effect=CHAT_CAPTURE.CaptureError("receipt failure")
        ):
            self.assertEqual(CHAT_CAPTURE.main(), 2)
    ...
    def test_position_must_answer_what_it_overturns(self) -> None:
        ...
        self.assertIn("identify the relation before writing", refused.stderr)
        self.assertEqual(self.recall_files(), [])
    ...
    def test_supersedes_binds_the_address_to_a_fingerprint(self) -> None:
        ...
        self.run_capture(
            ...
            supersedes=f"{target.name}#L{line_number} sha:{CHAT_CAPTURE.fingerprint(lines[line_number - 1])}",
        )
        written = target.read_text(encoding="utf-8")
        self.assertRegex(written, rf"supersedes: {target.name}#recall-[0-9a-f]{{32}}")
        ...
        self.assertIn("unverified line address", missing.stderr)
    ...
    def test_runtime_body_routes_to_existing_modes(self) -> None:
        skill_text = SKILL.read_text(encoding="utf-8")
        # Meaning is checked by agent review, not by freezing the author's prose.
        for mode in ("capture", "retrieval", "integrity"):
            relative = f"references/{mode}.md"
            self.assertIn(relative, skill_text)
            self.assertTrue((SKILL.parent / relative).is_file())
        self.assertIn("`_ops/chat-recall/`", skill_text)

    def test_modes_expose_their_own_tools_without_reference_chains(self) -> None:
        tools_by_mode = {
            "capture": ("chat_capture.py",),
            "retrieval": ("chat_digest.py",),
            "integrity": ("chat_digest.py", "chat_recall.py"),
        }
        for mode, scripts in tools_by_mode.items():
            content = (SKILL.parent / "references" / f"{mode}.md").read_text()
            for script in scripts:
                self.assertIn(script, content)
                self.assertTrue((SKILL.parent / "scripts" / script).is_file())
            self.assertNotIn("references/capture.md", content)
            self.assertNotIn("topic_reconcile.py", content)
        repair = (SKILL.parent / "references" / "integrity.md").read_text()
        self.assertIn("`capture-needed`", repair)

    def test_modes_address_the_runtime_corpus(self) -> None:
        integrity = (SKILL.parent / "references" / "integrity.md").read_text()
        self.assertIn(RUNTIME_ROOT_LINE, integrity)
        self.assertIn('TARGET_PROJECT_ROOT="${TARGET_PROJECT_ROOT:-$PWD}"', integrity)
        capture = (SKILL.parent / "references" / "capture.md").read_text()
        self.assertIn("--list-metadata", capture)
        self.assertIn("chat_capture.py", capture)
        self.assertNotIn("chat_digest.py", capture)
        retrieval = (SKILL.parent / "references" / "retrieval.md").read_text()
        self.assertIn(RUNTIME_ROOT_LINE, retrieval)
        self.assertIn('TARGET_PROJECT_ROOT="${TARGET_PROJECT_ROOT:-$PWD}"', retrieval)
        self.assertIn("chat_digest.py", retrieval)
        self.assertTrue((SKILL.parent / "scripts" / "chat_digest.py").is_file())
        self.assertIn("--topic", retrieval)
        self.assertIn("--timeline", retrieval)
        self.assertNotIn("chat_recall.py", retrieval)
        self.assertTrue((SKILL.parent / "references" / "search-mechanics.md").is_file())
