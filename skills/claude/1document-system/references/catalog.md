...
- Task → `1planning` · Finding → `1findings` · Rule/Instruction →
  `1instruction-shaping` · Product Frame / Principles / their journal →
  `1product-shaping`.
...
free projection. A skill-owned artifact family (e.g. Product Frame/Principles
pairs of `1product-shaping`) is never a project-local type candidate: route
to the owning skill instead of admitting a type. Never change the global
catalog without an explicit user request.
