...
видимым. Совпавшую визуальную анатомию даёт `details.collapse`; native element
владеет behavior, Daisy — shell. В одной группе не ставь `open` более чем одному
элементу и не вкладывай одноимённую группу в себя.
...
Расположи popover рядом с trigger в DOM; trigger и содержимое собирай из
совпавших Daisy button/menu/card roles. Обязательное содержание и длинную задачу
оставь на странице либо в `<dialog>`.

### Dialog

`<dialog>.showModal()` — редкая сфокусированная задача с modal semantics.
Используй Daisy `modal` anatomy поверх native `<dialog>`, явно выбери начальный
focus через `autofocus`. Закрытая страница остаётся понятной без содержимого
dialog.
