...
`classDef fill:var(--color-primary)` не «переопределяет тему» — он роняет
парсер: значение стиля читается до запятой, а скобки `var(...)` разбор ломают.
Схема тогда не рендерится вовсе, и на странице остаётся сырой текст
определения. Ошибка молчаливая: консоль чистая, узел получает
`data-mermaid-error="parse"`. Различай роли формой узла (`([стадион])`,
`[[рамка]]`, `{{ромб}}`), `stroke-width` и `stroke-dasharray` — их парсер
принимает.
