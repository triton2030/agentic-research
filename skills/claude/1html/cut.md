...
- Код — `artifact-card`/`artifact-verdict` голыми, без `card`+`card-body`
  (все отступы темы живут там) → шпаргалка теперь показывает несущую цепочку
  классов; в `theme.css` добавлена страховка `:not(:has(> .card-body))`.
...
- Рассмотрено и отклонено: учить `audit_html_style.py` ловить голые карточки —
  страховочный CSS убирает сам класс поломки; скрипт остаётся advisory.
