...
## 2026-09-23 — Визуальный Разговор И Лёгкая Переделка

Owner evidence: `_ops/chat-recall/2026-09-23-054205-claude-515c6ba8.md`
(четыре записи: цель скила, визуальное общение и раскрытие, красота и
художественность, поручение сравнить и исправить по рекомендации агента).
Карта изменений — `cut.md`, раздел того же дня.

### Автораскладка React Flow

| Claim | Falsifier | Статус |
|---|---|---|
| Узлы без `position` раскладываются без наложений | на `file://` при 1280 px узлы пересекаются или выходят за рамку | pass: `/tmp/1html-rf-test/rf-auto.html` — LR 6 узлов разной высоты и 7 связей, TB 4 узла; 0 пересечений, 0 вне рамки, 0 ошибок, 0 сетевых запросов |
| Поздние стили не ломают раскладку | узлы, выросшие после Tailwind, наезжают или вылезают | pass после исправления: первый вариант подгонял вид до применения координат и до стилей Tailwind — 4 узла вне рамки; раскладка теперь повторяется при смене размеров, вид считается по своим границам |
| Ручные координаты работают как раньше | страница с `position` меняет геометрию на новой сборке | pass: `2026-08-20-visual-systems-lab.html` и `_smoke.html` mavo-short2 на старой и новой сборке дают одинаковые позиции и размеры, 0 ошибок |
| Сборка — те же пакеты плюс dagre | другие версии React Flow в бандле | частично: пересборка из тех же закреплённых версий побайтно со старым бандлом не совпала (другая обёртка модулей esbuild); поведение сверено браузером |

Узкий экран (390 px): как и раньше, вид не ужимается до миниатюры — первый
узел в читаемом масштабе, остальные доступны панорамированием.

### Парный прогон

Одна просьба владельца, свежий агент на каждую версию, один запуск:
«/1html объясни мне, как браузер решает, взять страницу из своего кэша или
снова спросить сервер. Я не понимаю, что такое ETag и ответ 304.» Замер —
Chromium по `file://` на 1440 и 390 px.

| Версия | Видимых слов | В раскрытии | Управление | Повторы из данных | Ошибки | Вбок на 390 px | Время / действий |
|---|---|---|---|---|---|---|---|
| прежняя | 569 | 43 | 9 | 0 | 0 | нет | 32 мин / 154 |
| первая редакция, 120 строк с пятью правилами кода | 566 | 70 | 19 | 5 | 0 | нет | 38 мин / 86 |
| итоговая, 67 строк: цель и дельта | 436 | 200 | 14 | 6 | 0 | 7 px своей вёрстки страницы | 32 мин / 86 |

- Итоговая редакция без явных правил кода дала страницу не беднее первой:
  меньше видимого текста, подробности в раскрытии, песочница с шагами,
  повторы из данных. Это возможность, а не надёжность: один запуск, одна тема.
- Агент первой редакции скопировал в свою зону `components.css`, исправленный
  во время его работы; агент итоговой нашёл в справочнике Alpine обещание
  стиля `x-cloak`, которого в зоне не было, — стиль добавлен, справочник снят.
- Пробы поправки («добавь CDN») остановлены по слову владельца «много
  субагентов вызывать не обязательно»: свойство «поправка — короткая правка»
  проверено только косвенно.
- Первая версия метрики «в раскрытии» считала текст `<script>` с данными;
  цифры выше — после исправления.

### Итоговая проверка зоны

- Свежая зона скриптом кандидата: плагины Intersect и Collapse, `x-cloak`,
  иконки Lucide внутри `x-for`, React Flow без координат — работают; 0
  предупреждений и ошибок консоли.
- `.page` на копиях живых страниц: колонка текста на 390 px — 350 px вместо
  117 px, без прокрутки вбок; на 768 и 1280 px — 728 и 1056 px.
- Существующая зона со старым рантаймом: создание страницы скриптом кандидата
  обновляет рантайм React Flow; ручные `AGENTS.md`, `COMPONENTS.md` и шаблон
  зоны не перезаписываются.

## 2026-09-23 — Быстрая Отдача, Команда Проверки, Alpine 3.17.4

### Проба скорости

Та же просьба про кэш и ETag, свежий агент, один запуск, копия скила с
абзацем «отдавай, как только открывается без ошибок» и командой
`check_html.mjs`. Замер — тот же `measure.js`, Chromium по `file://`.

| Версия | Видимых слов | В раскрытии | Управление | Повторы из данных | Ошибки | Вбок на 390 px | Время / действий |
|---|---|---|---|---|---|---|---|
| итоговая утренняя | 436 | 200 | 14 | 6 | 0 | 7 px | 32 мин / 86 |
| с быстрой отдачей | 374 | 156 | 12 | 5 | 0 | нет | 4 мин / 15 |

Страница не беднее: тренажёр с ползунком времени, правкой файла на сервере и
переключателем `Cache-Control`, подсвеченное дерево решения, заголовки при
наведении, аналогия помечена как своя. Один запуск, одна тема: это
возможность, а не надёжность.

### Alpine В Скрытых Блоках

| Утверждение | Опровержение | Статус |
|---|---|---|
| Mermaid в скрытом блоке рисуется при показе | после показа `svg` уже 16 px | pass после правки адаптера: `x-show`, закрытый `<details>`, `x-collapse`, вкладка DaisyUI — 548 px; до правки — 16 px |
| ECharts в скрытом блоке рисуется при показе | нет `svg` или ширина 0 после показа | pass без правки: 1054–1056 px во всех трёх видах скрытия |
| `x-intersect.dwell.500ms` не срабатывает на пролёте | счётчик растёт после 200 мс в кадре | pass: 0 после пролёта, 1 после 900 мс |
| Директивы неподключённых плагинов молчат | — | fail: Alpine пишет предупреждение; формулировка справочника исправлена, `check_html.mjs` теперь ловит предупреждения Alpine, включая `Alpine Expression Error` |
| Живые страницы работают на 3.17.4 | ошибка или предупреждение Alpine на любой странице | pass: agentic-research 6, civicchain-v2 3, mavo-short2 5 страниц |
