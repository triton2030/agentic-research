...
Их нельзя перепутать, и брать нужно всегда одну из них — сырой токен бренда
как цвет текста на бумаге не читается: жёлтый на белом даёт 1,57:1.

| Нужно | Бери | Что гарантировано |
|---|---|---|
| залить блок | `.surface-*`, `.tile-*` или Daisy-компонент с ролью (`badge-primary`, `btn-primary`, `alert-*`) | текст внутри уже правильного цвета |
| покрасить текст на бумаге | `.ink-brand` `.ink-second` `.ink-strong` `.ink-quiet` | всегда читается |

Компонент DaisyUI несёт пару surface/content сам — поэтому `badge-primary`
безопасен, а `text-primary` небезопасен. Если тянет к утилите цвета, значит
нужен компонент или поверхность.
