...
вместе): Фейнман — сложное простым языком; «Made to Stick» — конкретика
против проклятия знания и любопытство; Билл Брайсон и «What If» Манро —
бытовая аналогия, лёгкая шутка, живой рассказ. Имена — ярлыки: сюда входит
только названная ось; на самой странице имена методов и книг не пишутся.
Вёрстка, порядок разделов, эмодзи и таблицы из «Формы Ответа» не берутся —
ими владеют [readable-design](references/readable-design.md) и «План До
Кода».

- Правила предложений — копия /123-explain, правь вместе: одна мысль — одно
  предложение; действие глаголом; абстракцию — конкретным «было → стало»;
  каждый термин пояснён рядом.
- Трудный смысл несёт бытовую аналогию рядом — подписью, сравнением или
  inline SVG.
- Заголовок-вопрос допустим по границе карточки «Заголовки Как Outline»
  ([readable-design](references/readable-design.md)).
- Шутка и игра — приправа в подписях и пустых состояниях, не в данных;
  факты и числа остаются точными.
...
- **блок → отношение → носитель.** Перечисли блоки страницы. Каждому — какое
  ...
  Проза законна только как явно записанное «отношения нет — проза».
...
Проза — последний носитель, а не дефолт: абзац допустим только для смысла,
в котором нет показываемого отношения, и это решение видно в плане — в
присвоении «блок → отношение → носитель». Прозе остаются подпись, связка,
короткий вывод и текстовое резюме визуала — то, что владельцы носителей
требуют текстом. Недобравший носитель поднимай по лестнице
[visual-routing](references/visual-routing.md) до названного там порога;
выше порога не поднимай.
...
удобную версию. Комментарий агента — риск, гипотезу, вопрос, вывод или
аналогию — назови и отдели от фактов. Существенный смысл оставь в semantic DOM: headings,
...
термины, которых читатель может не знать, пояснены рядом; трудный механизм
несёт помеченную аналогию; страница выражает собственный visual intent и автономно открывается по первой
