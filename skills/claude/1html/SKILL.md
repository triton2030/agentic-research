...
- **блок → что на узком.** Столбцы схлопываются, таблица прокручивается внутри
  ...
  [layout-primitives](references/layout-primitives.md) → примитив, который
  переносится сам, и форма порога.
...
- **свой класс → отношение.** Поимённый список авторских классов; каждый
  назван за отношение — композицию, шов, канву, — не за вид. В CSS существуют
  только классы из списка; «ещё один стиль текста» классом не является.
