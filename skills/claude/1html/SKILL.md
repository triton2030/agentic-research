...
## Одна Информационная Работа

До компонентов зафиксируй три вещи: с каким вопросом читатель пришёл, какой
короткий ответ даёт страница и что читатель сможет понять, решить или сделать
после неё. Одна HTML-страница обслуживает одну такую работу. Если объяснение,
техническая спецификация, реестр и приёмка нужны разным чтениям, оставь их в
одном artifact project, но разнеси по внутренним страницам.

Для explainer, decision, plan и tutorial строй связный вопросный хребет:
ориентация → ответ → почему / evidence / contrast → следствие или действие.
Каждый видимый раздел отвечает на вопрос, который естественно возник из
предыдущего. Не собирай страницу как перечень найденных тем или демонстрацию
компонентов. Каталог, dashboard и reference — допустимые исключения: их единая
работа действительно состоит в поиске, мониторинге или сравнении модульных
элементов.

Progressive disclosure добавляет глубину к уже видимому тезису, а не прячет
сломанный порядок или вторую задачу. Основной ответ, минимально достаточная
опора, критический риск и следующий ход остаются видимыми.

Если композиция использует cards, tabs, accordion, anchors или несколько
скрытых слоёв, до разметки открой
[`references/daisy-storytelling.md`](references/daisy-storytelling.md). Он
владеет точной картой reader jobs, компонентов и связного раскрытия.
...
версии, а не только знакомый короткий набор. Полный доступ не означает показ
ассортимента: используй минимальный набор, который передаёт связи именно этой
страницы. Начинай с DaisyUI component (`card`, `badge`, `menu`, `btn`, `alert`,
`collapse`, `tabs`, `table` и остальные), применяй semantic colors (`primary`,
`base`, `success`, `warning`, `error`), затем добавляй Tailwind utilities для
конкретной композиции. Не закрепляй случайные palette colors и SVG fill/stroke,
если ту же роль выражает тема. Стабильная повторяемая роль принадлежит
`theme.css`; одноразовое расположение может остаться utility-классом.
...
Ближайший пример связного explainer с несколькими смысловыми ходами —
`references/daisy-examples/05-creative-story.html`.
...
- каждая страница выполняет одну информационную работу, а её раскрываемые слои
  продолжают видимый тезис;
