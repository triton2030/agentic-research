...
есть `index.html`; `_catalog/` хранит его общие локальные стили и runtime.
...
full-screen содержанием, сократи prose и используй SVG, если связи важнее
карточек. Click, animation и смена темы нужны только когда помогают чтению;
цвета SVG держи в CSS variables.
...
- корневой каталог обновлён и ведёт на новый artifact;
