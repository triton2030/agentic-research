...
## Автовызов — 2026-09-25

С 2026-08-20 скил вызван в 3 из 2080 сессий Claude и открыт в 120 из 3631
сессий Codex. Сессий Claude, где выполнялись `npm install`, `tsc`, `gh`,
`vercel`, `ffmpeg` и `uv tool`, — около 50. Причина: условие «version-sensitive
choice» требует от агента знать, что его память устарела. Description
переписана на наблюдаемые команды; ловушки npm 12 и отката npm вынесены в тело.
Повторить подсчёт после месяца работы.

## Перечень и глобальная строка — 2026-09-25

Три свежих агента Sonnet 5 без упоминания скила:

- подсчёт вызовов скила по JSONL-сессиям — скил не вызван дважды, агент взял
  `find` + `grep` (первый прогон ещё видел прежнюю description);
- таблица и массовая замена поля во frontmatter — скил вызван, выбраны `yq` и
  `sd` из перечня;
- ранее тем же днём: `npm install sharp` — скил вызван, применена дельта npm 12.

Вывод: момент «пишу помощник или выбираю CLI» срабатывает; задачу, которую
агент считает решаемой привычным `grep`, он выбором инструмента не считает.
