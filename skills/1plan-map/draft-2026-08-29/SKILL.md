...
4. Записывай только принадлежащую карте правду по
   ...
   [дашборду](references/dashboard.md). Форма карты владеет общим словарём
   статусов и нормализацией `эпик-снимок`; task-форма лишь повторяет и
   потребляет их. Не складывай сюда цель, критичный контекст или ограничения
   исполнения задачи: фронтир без живой задачи лишь вызывает допуск через
   `1planning`, но не разрешает создать её.
