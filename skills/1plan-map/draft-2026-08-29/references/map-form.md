...
Токены схемы литеральны: frontmatter-ключи и заголовки разделов проверяются
байт-в-байт и читаются владельцем. Эта форма владеет общим словарём статусов
эпиков и задач; task-форма повторяет его для чистого исполнителя, а прибор
проверяет равенство. Содержимое файла пишется на языке проекта.
...
его проверяет. Эта форма владеет нормализацией снимка: если проектный прибор не
задал иную, значимое содержимое — frontmatter без производных полей плюс тело
вне «Апдейтов».
...
владеет `1plan-task`, включая статус из общего словаря.
