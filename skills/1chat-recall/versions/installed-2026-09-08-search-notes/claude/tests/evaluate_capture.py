...
def main() -> int:
    ...
    report["retrieval_of_support"] = {
        "note": (
            "опора записана — находит ли её продукт. Формулировки сравниваются "
            "между собой; прирост от второй НЕ переносится на агента, не "
            "знающего адреса: слепая вторая формулировка даёт 0.673 против "
            "0.776 у одиночного конформного запроса (замер 2026-08-20)."
        ),
