...
class ChatRecallTests(unittest.TestCase):
    ...
    def test_plain_user_visible_tool_followup_is_not_direct_owner_speech(self) -> None:
        direct, followup, current = (str(uuid.uuid4()) for _ in range(3))
        self.write(
            [
                self.record(
                    direct,
                    None,
                    "user",
                    {"role": "user", "content": "Прямые слова владельца."},
                ),
                self.record(
                    followup,
                    direct,
                    "user",
                    {"role": "user", "content": "Сообщение другого агента."},
                    sourceToolAssistantUUID=str(uuid.uuid4()),
                ),
                self.record(
                    current,
                    followup,
                    "user",
                    {"role": "user", "content": "Текущий ход."},
                ),
            ]
        )

        output = self.call("--all").stdout
        self.assertIn("Прямые слова владельца.", output)
        self.assertNotIn("Сообщение другого агента.", output)
