#!/usr/bin/env python3
"""Recover provenance-filtered user-message candidates from a Codex transcript."""
...
AUTO_LATEST_COUNT = 5
CODEX_DELEGATION_RE = re.compile(r"^\s*<codex_delegation\b", re.IGNORECASE)
DIRECT_PROVENANCE_VALUES = {"direct", "human", "owner", "user"}
PROVENANCE_KEYS = (
    "author",
    "carrier",
    "delivery",
...
PROVENANCE_MAX_DEPTH = 64
PROVENANCE_MAX_NODES = 100_000
_PROVENANCE_OVER_LIMIT = object()
...
def response_item_text(payload: dict[str, Any]) -> str | None:
    """Read text input from the current Codex user-message record shape."""
...
def _provenance_is_direct(value: Any) -> bool:
    if isinstance(value, str):
        return value.casefold() in DIRECT_PROVENANCE_VALUES
    if isinstance(value, dict):
        candidates = [value.get(key) for key in ("kind", "type", "name")]
        return any(
            isinstance(candidate, str)
            and candidate.casefold() in DIRECT_PROVENANCE_VALUES
            for candidate in candidates
        )
    return False


def _provenance_values(value: Any):
    """Yield provenance markers without recursive Python calls."""
    stack: list[tuple[Any, str | None, int]] = [(value, None, 0)]
    seen_containers: set[int] = set()
    nodes = 0
    while stack:
        current, key, depth = stack.pop()
        if key is not None and key in PROVENANCE_KEYS:
            yield current
        nodes += 1
        if nodes > PROVENANCE_MAX_NODES:
            yield _PROVENANCE_OVER_LIMIT
            return
        ...
        if isinstance(current, dict):
            for nested_key, nested in reversed(list(current.items())):
                stack.append((nested, nested_key, depth + 1))
        else:
            for nested in reversed(current):
                stack.append((nested, None, depth + 1))


def has_unconfirmed_carrier(
    record: dict[str, Any], payload: dict[str, Any], text: str
) -> bool:
    """Reject known delegation wrappers and any non-human provenance marker."""
    if CODEX_DELEGATION_RE.match(text):
        return True
    for container in (record, payload):
        for value in _provenance_values(container):
            if not _provenance_is_direct(value):
                return True
    return False


def response_item_user_text(
    record: dict[str, Any], payload: dict[str, Any], warnings: list[str]
) -> str | None:
    if payload.get("type") != "message" or payload.get("role") != "user":
        return None
    metadata = payload.get("internal_chat_message_metadata_passthrough")
    kinds = metadata.get("content_item_kinds") if isinstance(metadata, dict) else None
    if not isinstance(kinds, list) or "user.text" not in kinds:
        return None
    ...
    if not text:
        return None
    if has_unconfirmed_carrier(record, payload, text):
        warnings.append(
    ...
    for sequence, record in enumerate(jsonl_records, start=1):
        ...
        if record_type == "event_msg":
            ...
            if event_type == "user_message":
                ...
                if has_unconfirmed_carrier(record, payload, text):
                    warnings.append(
                        "Ignored an event_msg user message without direct-user "
                        "provenance"
                    )
                    continue
        ...
        response_metadata = payload.get("internal_chat_message_metadata_passthrough")
        response_turn = current_turn
        if isinstance(response_metadata, dict):
            metadata_turn = response_metadata.get("turn_id")
            if metadata_turn is not None:
                if not isinstance(metadata_turn, str):
                    warnings.append("Ignored a response_item with an invalid turn_id")
                    response_turn = None
                else:
                    response_turn = metadata_turn

        if item_type == "message":
            text = response_item_user_text(record, payload, warnings)
            if text is None or response_turn is None:
                continue
            message_ordinals[response_turn] += 1
            raw_messages.append(
                RawMessage(
                    turn_id=response_turn,
                    timestamp=require_timestamp(
                        record.get("timestamp", ""), "response_item.message.timestamp"
                    ),
                    text=text,
                    sequence=sequence,
                    ordinal=message_ordinals[response_turn],
                )
            )
            continue
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Recover provenance-filtered user-message candidates and Plan answers "
    ...
    read_parser = subparsers.add_parser(
        "read", help="read user-message candidates chronologically"
    )
    ...
    search_parser = subparsers.add_parser(
        "search", help="search provenance-filtered transcript candidates"
    )
