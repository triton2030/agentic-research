...
class ChatCaptureTests(unittest.TestCase):
    ...
    def test_modes_address_the_runtime_corpus(self) -> None:
        ...
        # The corpus is read only by the psychologist role; the caller does not search.
        skill_text = SKILL.read_text(encoding="utf-8")
        self.assertIn("agents/psychologist.md", skill_text)
        psychologist = (SKILL.parent / "agents" / "psychologist.md").read_text()
        self.assertIn(RUNTIME_ROOT_LINE, psychologist)
        self.assertIn('TARGET_PROJECT_ROOT="${TARGET_PROJECT_ROOT:-$PWD}"', psychologist)
        self.assertIn("chat_digest.py", psychologist)
        self.assertTrue((SKILL.parent / "scripts" / "chat_digest.py").is_file())
        self.assertNotIn("chat_recall.py", psychologist)
        self.assertIn("--topic", psychologist)
        self.assertIn("--timeline", psychologist)
        retrieval = (SKILL.parent / "references" / "retrieval.md").read_text()
        self.assertNotIn("chat_digest.py", retrieval)
