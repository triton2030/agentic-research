#!/usr/bin/env python3
"""Print provenance-filtered user-message candidates from a Claude Code session."""
...
def record_timestamp(record: dict[str, Any]) -> str:
    ...
    if not isinstance(raw, str) or not raw:
        raise RecallError("user-message candidate has no source timestamp")
    try:
        parsed = datetime.fromisoformat(raw.replace("Z", "+00:00"))
    except ValueError as error:
        raise RecallError(
            "user-message candidate has an invalid source timestamp"
        ) from error
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise RecallError("user-message candidate source timestamp has no timezone")
