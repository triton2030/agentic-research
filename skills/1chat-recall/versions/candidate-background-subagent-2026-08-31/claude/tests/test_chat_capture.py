...
class ChatCaptureTests(unittest.TestCase):
    ...
    def test_runtime_owner_documents_metadata_vocabulary(self) -> None:
        ...
        self.assertIn("карту target", capture_text)
    ...
    def test_capture_contract_gates_on_materiality(self) -> None:
        ...
        normalized_repair = " ".join(repair_text.split()).lower()
        self.assertIn("из ранее сохранённых цитат", normalized_skill)
        self.assertIn(
            "обычный поиск читает только сохранённый корпус",
            normalized_skill.lower(),
        )
        self.assertIn("User-visible carrier не доказывает авторство", normalized_skill)
        self.assertIn("Явно заказанные Validation, Repair или backfill", skill_text)
        self.assertIn("`capture-needed` сначала проходит Capture", skill_text)
        self.assertIn("простое assent пропусти", capture_text)
        self.assertIn("выбранный `selection`", capture_text)
        self.assertIn("agent follow-up", capture_text)
        self.assertIn("только удалением", capture_text)
        self.assertIn("вставленные документы", capture_text)
        self.assertIn("слова других людей", capture_text)
        self.assertIn("или агентов", capture_text)
        self.assertIn("--list-metadata", capture_text)
        self.assertIn("если карты нет, верни gap", capture_text)
        self.assertIn("noun-phrase index", capture_text)
        self.assertIn("`capture-needed`", repair_text)
        self.assertIn("provenance сообщения", normalized_repair)
        self.assertIn(
            "не доказывает авторство каждого фрагмента", normalized_repair
        )
        self.assertIn("иначе не читай transcript и верни gap", normalized_repair)
        self.assertIn("owner request в текущем ходе", repair_text)
    ...
    def test_retrieval_contract_keeps_metadata_routes_and_full_holder_read(self) -> None:
        ...
        self.assertIn("target corpus", retrieval_text)
        self.assertIn("`topic_candidates`", retrieval_text)
        self.assertIn("`holders`", retrieval_text)
        self.assertIn("session holder целиком", retrieval_text)
        self.assertIn("поздними словами или live owner", retrieval_text)
        self.assertIn("ровно одного", retrieval_text)
        self.assertIn("дешёвого доступного", normalized_retrieval)
        self.assertIn("нативный механизм текущего runtime", normalized_retrieval)
        self.assertIn("`Agent`", retrieval_text)
        self.assertIn("`subagent_type: general-purpose`", retrieval_text)
        self.assertIn("`context: fork`", retrieval_text)
        ...
        self.assertIn("продолжай исходную работу", normalized_retrieval)
        ...
        self.assertIn("продолжай собственный поиск", normalized_retrieval)
