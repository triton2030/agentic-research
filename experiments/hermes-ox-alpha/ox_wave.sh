...
#
# MODE=read  — только чтение, брифы возвращают вердикт в JSON. Дефолт и
#                 самый безопасный режим: агент не касается дерева вовсе, а
#                 результат кладёт оркестратор. Ему же достаётся сверка баланса.
# MODE=write    — агенты пишут прямо в дерево. Локальная инструкция этой папки
#                 (AGENTS.md) требует для пишущих агентов волны отдельный
#                 worktree на каждого. Значит write допустим только когда
#                 участки заведомо не пересекаются по файлам и это проверено
#                 предполётом; во всех прочих случаях бери worktree.
