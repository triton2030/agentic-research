...
- `page_fit.answering_record_ids` that directly answer that H1;
...
- a reject reason says why the record contains no settled reusable knowledge;
...
Do not use any rejected candidate or pre-v1 Wiki prose as semantic input. The
first v1 batch starts from an empty Wiki; later batches use only the accepted
v1 chain.
