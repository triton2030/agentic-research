...
**Указатель — производная, и после любой правки страниц он врёт.** Добор,
структурные правки и правки формулировок меняют состав, а оглавление остаётся
прежним: страница есть, маршрута к ней нет. Проверка формы это ловит строкой
«маршрутов из индекса 0», но ловит уже постфактум. Дешевле пересобирать:
скелет и `apply_index.py` заново, ответ модели о названиях разделов
переиспользуется и второго прогона не требует.
