...
# Четыре рукава меряют четыре разные вещи, и ни один не лишний:
# `library` — проверяемое; `topics` — слой на стадию раньше, который уже несёт
# те же пункты дословно и потому спрашивает, заработана ли нарезка на страницы;
# `digest` — действующий маршрут скила 1chat-recall; `bare` — пол.
WHERE = {
    ...
    "topics": "Тебе доступны только тематические файлы знания: рабочая папка и есть их "
              "корень. Каждый файл — одна тема, внутри пункты с якорями на источник. "
              "Указателя нет: ориентируйся по именам файлов и заголовкам внутри. "
              "Якоря наружу не открываются и ответа не требуют.",
}
MODE = {
    "library": "путь страницы, на которой нашёл ответ",
    "corpus": "имя файла и номер строки, где нашёл ответ",
    "topics": "имя файла темы и пункт, в котором нашёл ответ",
}
