...
def _safe_print(message: str, *, stream: Any = None) -> None:
    # Часы владельца рядом с событием волны. Панель показывает порядок строк,
    # но не время, а сверять волну приходится с внешним миром: коммитом,
    # соседним прогоном, собственной памятью «когда я это запустил». Время
    # локальное и без даты — волна короче суток, дата была бы шумом.
    if message.startswith(ORCH_PREFIX):
        message = (
            f"{ORCH_PREFIX} {time.strftime('%H:%M:%S')}"
            + message[len(ORCH_PREFIX):]
        )
...
def _tier_overrides(tasks: list[TaskSpec]) -> str:
    ...
    Без неё строка `model=gpt-6-sol` — ложь для половины воркеров: ярус теперь
