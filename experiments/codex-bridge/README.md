...
`effort=medium` — основной ярус (см. «Ярусы вызова» в `AGENTS.md`). Модель и
...
— `medium`; `xhigh` и выше — осознанный opt-in под задачи, где надо думать. Как и у `service_tier`, ledger фиксирует
