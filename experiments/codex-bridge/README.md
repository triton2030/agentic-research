...
**Витрина владельца — та же фоновая Bash-карточка, что и запуск.** Правило
владельца (2026-08-24: «чтобы в десктоп приложении я видел факт того что
агенты работают, типа как баш команды»; 2026-09-18: агент запускает без
витрины, а карточка с заданием в командной строке «выглядит мусорно»):
`codex_launch.py <review|investigate|orchestrate> --name … --prompt-file … --
<аргументы входа>` запускает вход моста отдельным процессом с выводом в
`<RUN_DIR>.launch.log`, а свой stdout отдаёт `codex_watch.py watch RUN_DIR
--pulse`: заголовок с моделью/усилием и заданием из `prompt.md`, дальше по
строке на каждое сообщение Codex и на каждую сводку его размышлений
(`reasoning.summary`; в нативном ревью это единственные слова до финала) —
время от старта и начало текста (владелец 2026-09-18: «вижу его
мысли … чисто его текстовые слова»). Команды, файлы, инструменты и поиск в
витрину не идут; `--pulse-all` показывает и их со значками (📖 чтение · ⌘
команда · ✎ файл · 🔧 инструмент · 🌐 поиск · 🤖 субагент) без обёртки
`/bin/zsh -lc` — режим отладки моста. Панель приложения рисует это в реальном
времени, в контекст агента строки не попадают. `TaskStop` карточки останавливает
и прогон — это делает сам `codex_launch.py`: пересылает SIGTERM/SIGINT/SIGHUP
входу моста, тот просит у движка штатный interrupt (`interrupt_requested` в
`events.jsonl`, provisional `result.json`); витрина кончилась раньше прогона
(не встала, потолок часов) — прогон гасится после grace-периода. Аудит Codex
2026-09-18 отверг расчёт на харнесс. `RUN_DIR` выбирается заранее (`--run-dir`);
`watch` ждёт появления каталога (`--wait-sec`, 180 с) и завершается вместе с
прогоном — одно уведомление агенту. Без `--pulse` `watch` печатает только
завершения — это режим для нативного `Monitor` у волны, где каждая строка
ложится в контекст агента (решение владельца 2026-08-24: будить только по
завершению).
...
Реплика в идущий ход — `codex_progress.py RUN_DIR --steer "…" [--worker ID]`
кладётся в ящик прогона и доставляется сторожем как `steer` с правами
пользователя. `--external` помечает текст, который агент лишь передаёт
(файл, другой агент, вывод инструмента): сторож вводит его в ход через
`thread.turn(ExternalMessage(...))` — join активного хода с правами
инструмента (SDK ≥ 0.154.0, `openai/codex#44086`); своё исправление курса так
не помечают, оно проиграет исходному заданию. В `events.jsonl` у
`steer_requested` / `steer_accepted` поле `authority`.
...
поломки, а ради двух возможностей SDK `0.154.0` (`#44084`, `#44086`) и потому,
...
единственный ручной `TurnHandle` (нативное ревью `--mode diff`) подписывается
после `review/start` и может не увидеть первые события хода — на результат это
не влияет, `TurnResult` собирается по `turn/completed`. Открытых enum'ов 3 из
