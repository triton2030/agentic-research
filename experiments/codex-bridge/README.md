...
витрины): прогон стартует в subshell с выводом в `<RUN_DIR>.launch.log`, а
stdout карточки занимает `codex_watch.py watch RUN_DIR --pulse` — строка на
каждый шаг работы (команда, файл, инструмент, ответ; reasoning и сам запрос
пропускаются), панель приложения рисует это в реальном времени, в контекст
агента строки не попадают. `RUN_DIR` выбирается заранее (`--run-dir`);
