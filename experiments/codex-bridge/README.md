...
роем параллельных агентов), `terra` — средняя. Рабочий пол для `luna` —
`--effort medium` (вердикт владельца 2026-07-13: на `low` слишком тупая для
пользы; `low` — только технический пробник; `medium` живо проверен, те же
