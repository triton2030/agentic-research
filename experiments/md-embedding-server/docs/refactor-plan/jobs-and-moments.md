...
| Проверить корпус | Найти дубли, расползание смысла, discovery gaps | `md_audit`, `md_overlaps`, `md_repeated_concepts` |
| Проверить graph health | Найти broken links, cycles, schema drift | `md_health`, `md_check`, `md_cycles`, `md_scan` |
| Закрыть работу | Проверить изменённые `.md` и свежесть индекса | `md_changed`, `md_status` |
