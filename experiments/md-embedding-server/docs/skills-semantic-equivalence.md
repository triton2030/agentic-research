...
| `md_changed` | 7 | `md changed [--scan SCAN] [--depth DEPTH] [--base BASE] [--since SINCE] [--staged] [--path-include PATH_INCLUDE] [--path-exclude PATH_EXCLUDE] --json` |
| `md_strip` | 6 | `md strip [--paths PATHS] [--confirm] [--dry-run] [--transaction-id TRANSACTION_ID] [--also-related-section] [--path-include PATH_INCLUDE] [--path-exclude PATH_EXCLUDE] --json` |
...
- `md_preflight` / `md_changed` must never be summarized as safe/unsafe; they expose obligations and signals only.
...
- Legacy-only cluster fallback: keep `experiments/md-embedding-server/scripts/md_navigator.py cluster <folder> --k <expected-topics>` as a debug fallback until a real `md cluster` subcommand exists; do not rewrite it to `md audit` as if it were equivalent.
