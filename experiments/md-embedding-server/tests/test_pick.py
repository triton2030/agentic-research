...
def test_pick_search_results_are_not_a_map(tmp_path: Path) -> None:
    """`md extract` stays map-only; semantic search bodies live in `md search-read`."""
    out = pick_items(
        {"root": str(tmp_path), "results": [{"section_id": "1.2"}]},
        file_ids=set(),
        heading_ids={"1.2"},
        extract=True,
    )
    assert out["headings"] == []
    assert out["missing_heading_ids"] == ["1.2"]
