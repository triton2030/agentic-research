...
def test_catalog_contains_snapshot_entries() -> None:
    snapshot = json.loads((ROOT / "tests/golden/mcp-tool-snapshot.json").read_text())
    assert len(TOOLS) == len(snapshot["tools"])
    assert len(TOOLS_BY_ID) == len(snapshot["tools"])
...
def test_md_tools_json_returns_catalog_with_envelope() -> None:
    ...
    assert len(payload["tools"]) == len(TOOLS_BY_ID)
